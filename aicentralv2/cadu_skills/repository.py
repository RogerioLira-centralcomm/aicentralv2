...
def charge_project_rag(cursor, *, client_id: int, user_id: int, project_id: str,
    ...
    try:
        charged = CaduCreditConnector().charge_tokens_in_transaction(
            cursor,
            actor=CreditActor.from_values(client_id, user_id),
            idempotency_key=idempotency_key,
            app='workspace_rag', stage=stage, charged_tokens=tokens,
            model='postgres-text',
            metadata={'projeto_id': str(project_id), 'rag': 'postgresql',
