...
def route_request(message: str, surface: str = "conversations", has_project: bool = False,
    ...
    if has_project and _has(text, r"\b(?:listar|liste|mostrar|mostre|ver|quais|acompanhar)\w*\b.{0,45}\b(?:tarefas?|atividades?)\b"):
        return IntentRoute("workspace", "list_project_tasks", "low", "analysis",
                           ("project",), ("projects.list_tasks",))

    project_task_list = _has(
        text,
        r"\b(?:cri\w*|proponh\w*|mont\w*|organiz\w*|planej\w*)\b.{0,55}\b(?:lista\s+de\s+)?(?:tarefas?|atividades?)\b|"
        r"\b(?:primeira\s+)?lista\s+de\s+(?:tarefas?|atividades?)\b|\b(?:tarefas?|atividades?|pr[oó]ximos?\s+passos?)\b.{0,55}"
        r"\b(?:projeto|contexto|fontes?|conversas?)\b|\b(?:projeto|contexto|fontes?|conversas?)\b.{0,55}"
        r"\b(?:tarefas?|atividades?|pr[oó]ximos?\s+passos?)\b",
    )
