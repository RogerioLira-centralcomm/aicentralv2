...
def _workspace_brand_form() -> dict:
    ...
    def text_list(field: str, limit: int = 12) -> list[str]:
        return [
            ' '.join(item.split())[:300]
            for item in (request.form.get(field) or '').splitlines()
            if item.strip()
        ][:limit]
    palette = []
    ...
    profile = {
        ...
        'brand_summary': (request.form.get('brand_summary') or '').strip()[:4000],
        'products_services': text_list('products_services'),
        'differentiators': text_list('differentiators'),
        'proof_points': text_list('proof_points'),
        'creative_guidelines': (request.form.get('creative_guidelines') or '').strip()[:4000],
        'visual_motifs': text_list('visual_motifs'),
        'mandatory_elements': text_list('mandatory_elements'),
        'forbidden_elements': text_list('forbidden_elements'),
        'name_autogenerated': name_autogenerated,
    }
    # Palette and typography are advanced identity data and aren't rendered by
    # the compact edit dialog.  Omitting their controls must preserve the
    # approved values instead of replacing them with empty arrays.
    if 'color_palette' in request.form:
        profile['color_palette'] = palette
    if 'fonts' in request.form or any(f'font_{role}' in request.form for role in ('display', 'body', 'accent', 'legal')):
        profile['fonts'] = fonts
    # Kept only for older integrations that still post this legacy field.
    # The current Workspace UI uses operational brand signals instead.
    if 'brand_values' in request.form:
        profile['brand_values'] = text_list('brand_values')
    return {
        'name': name,
...
def _existing_brand_asset_url(value) -> str:
    """Return a public asset URL only when a local upload still exists.

    Brand rows outlive deploys, while generated/reference files can be removed
    during maintenance.  Never turn a stale database path into an image request
    in the Workspace UI: it creates console 404s and hides the real identity.
    Remote URLs remain valid evidence sources and are handled separately where
    a same-origin visual is required.
    """
    url = public_logo(value)
    if not url:
        return ''
...
def _brand_seed_visual_url(value, website_url: str = '') -> str:
    ...
    brand_host = (urlparse(str(website_url or '')).hostname or '').lower()
    if brand_host and (host == brand_host or host.endswith('.' + brand_host)):
        return url
    return ''
    ...
    try:
        ...
        with get_db().cursor() as cursor:
            ...
            for brand in brands:
                ...
                brand['display_logo'] = _existing_brand_asset_url(brand.get('resolved_logo_path'))
                brand['logo_variants'] = {
                    '64': _existing_brand_asset_url(brand.get('logo_64_path')),
                    '128': _existing_brand_asset_url(brand.get('logo_128_path')),
                    '256': _existing_brand_asset_url(brand.get('logo_256_path')),
                    '512': _existing_brand_asset_url(brand.get('logo_512_path')),
                }
                ...
                brand['visual_hero'] = _brand_seed_visual_url(
                    seed_visuals.get('hero') or seed_visuals.get('thumbnail'), brand.get('website_url')
                ) if isinstance(seed_visuals, dict) else ''
                ...
                for row in cursor.fetchall():
                    ...
                    for field in ('brand_profile', 'analysis_metadata'):
                        if isinstance(brand.get(field), str):
                            ...
                            brand[field] = {}
                    name = str(brand.get('name') or '').strip()
                    brand['display_logo'] = _existing_brand_asset_url(brand.get('logo_upload_path') or brand.get('logo_url'))
                    brand['visual_hero'] = ''
...
def _workspace_brand(client_id: int, brand_id: int) -> Optional[dict]:
    ...
    try:
        with get_db().cursor() as cursor:
            ...
            for asset in brand['assets']:
                ...
                stored_url = _existing_brand_asset_url(stored_path)
                is_local_upload = stored_path.startswith('/static/uploads/')
                # Do not fall back to a duplicate, missing local path.  This
                # covers both creative_references and creative_generated. A
                # bare external source is shown only when it belongs to the
                # declared official domain; all other candidates remain in
                # audit provenance, not as fragile UI image URLs.
                # The database is the durable source of truth. A local file may
                # be absent on one web instance while remaining available on
                # another instance or shared storage, so a read must never
                # delete the asset row.
                asset['display_url'] = stored_url or (stored_path if is_local_upload else source_url)
                asset['missing_file'] = is_local_upload and not bool(stored_url)
                if isinstance(asset.get('metadata'), str):
                    ...
                    asset['metadata'] = {}
                # A discovered logo is never allowed to replace or visually
                # compete with the approved identity unless it is hosted by
                # the declared official site.  Third-party footer marks are
                # evidence at most, not a brand logo.
                if (
                    str(asset.get('role') or '').lower() == 'logo'
                    and asset.get('display_url', '').startswith(('http://', 'https://'))
                    and not _brand_seed_visual_url(asset['display_url'], brand.get('website_url'))
                ):
                    asset['display_url'] = ''
                asset['missing_file'] = is_local_upload and not bool(stored_url)
    except Exception:
    ...
    brand['seed_visuals'] = {
        key: _brand_seed_visual_url(value, brand.get('website_url'))
        for key, value in seed_visuals.items() if value
    }
    ...
    weighted_fields = {
        'brand_summary': 9, 'positioning': 4, 'target_audience': 7,
        'tone_of_voice': 5, 'creative_guidelines': 5,
        'products_services': 4, 'differentiators': 3, 'proof_points': 3,
    }
    identity_score = sum(weight for field, weight in weighted_fields.items() if profile.get(field))
    ...
    evidence_score += 4 if profile.get('evidence_ledger') else 0
    approved_assets = [item for item in brand.get('assets', []) if str(item.get('status') or '').lower() == 'approved']
    assets_score = 4 if approved_assets else 0
    assets_score += 3 if any(str(item.get('role') or '') != 'logo' for item in approved_assets) else 0
    presence_score = 2 if profile.get('contacts') else 0
    presence_score += 1 if profile.get('addresses') else 0
    presence_score += 1 if profile.get('digital_policies') else 0
    presence_score += 1 if metadata.get('social_links') else 0
    breakdown = [
        {'id': 'identity', 'label': 'Identidade', 'score': identity_score, 'max': 48},
        {'id': 'visual', 'label': 'Sistema visual', 'score': visual_score, 'max': 25},
        {'id': 'evidence', 'label': 'Evidências', 'score': evidence_score, 'max': 15},
        {'id': 'assets', 'label': 'Ativos', 'score': assets_score, 'max': 7},
        {'id': 'presence', 'label': 'Presença pública', 'score': presence_score, 'max': 5},
    ]
    score = min(100, sum(item['score'] for item in breakdown))
    missing = []
...
def _merge_brand_analysis(brand: dict, analysis: dict, allowed_fields=None, *, overwrite=False) -> dict:
    ...
    allowed = set(allowed_fields or field_map.keys())
...
def _brand_analysis_proposal(analysis: dict) -> dict:
    ...
    allowed = {
        ...
        'ad_segments', 'creative_guidelines', 'campaign_opportunities', 'campaigns',
...
def _resolve_uploaded_logo_path(analysis: dict, brand: dict) -> Optional[str]:
    ...
    for asset in brand.get('assets') or []:
        ...
        if path:
            return path
    # Explicit logo uploads are saved as the client's logo before the audit
    # starts. If a previous read removed the legacy asset row, the durable
    # client pointer still identifies the exact first upload in this run.
    if int((upload or {}).get('order') or 0) == 1:
        durable_path = str(brand.get('logo_upload_path') or '').strip()
        if durable_path:
            return durable_path
    return None
...
def _brand_analysis_quality_score(analysis: dict, analysis_metadata: dict, central_review: dict) -> dict:
    """Return a stable 0-100 score without letting optional gaps veto the audit.

    Provider confidence is useful context, but observable output is the source
    of truth.  This makes the gate resilient when one reviewer is unavailable
    after collection already produced a useful, sourced dossier.
    """
    ...
    identity_weights = {
        ...
        'creative_guidelines': 4, 'products_services': 4,
        'differentiators': 3, 'proof_points': 2, 'audience_segments': 2,
    }
    visual_weights = {
        'logo_url': 7, 'primary_color': 5, 'secondary_color': 3,
        'color_palette': 4, 'fonts': 3, 'visual_motifs': 3,
    }
    marketing_weights = {
        'campaigns': 3, 'ad_segments': 2, 'competitors': 2,
        'personas': 1, 'campaign_opportunities': 2,
    }
    identity = sum(weight for key, weight in identity_weights.items() if present(key))
    visual = sum(weight for key, weight in visual_weights.items() if present(key))
    marketing = sum(weight for key, weight in marketing_weights.items() if present(key))

    sources = list(analysis.get('sources') or metadata.get('sources') or [])
    ledger = list(analysis.get('evidence_ledger') or [])
    pages = int(coverage.get('official_pages') or metadata.get('pages_analyzed') or 0)
    evidence = min(8, len(sources) * 2) + min(6, pages * 2) + min(6, len(ledger))

    try:
        review_confidence = max(0.0, min(1.0, float(review.get('confidence') or 0)))
    except (TypeError, ValueError):
        review_confidence = 0.0
    # A valid central opinion is worth at most ten points. Its absence reduces
    # the score, but never erases evidence already collected and normalized.
    review_score = round(review_confidence * 7)
    if str(review.get('status') or '') == 'ready':
        review_score += 3

    breakdown = {
        'identity': min(35, identity),
        'visual': min(25, visual),
        'evidence': min(20, evidence),
        'marketing': min(10, marketing),
        'review': min(10, review_score),
    }
    return {
        'score': min(100, sum(breakdown.values())), 'breakdown': breakdown,
        'version': BRAND_ANALYSIS_SCORE_VERSION,
    }
...
def _auto_apply_brand_analysis(client_id: int, user_id: int, brand_id: int, brand: dict, analysis: dict, decision: dict) -> dict:
    ...
    # Reviewer blocks are hard publication boundaries for every field.
    ...
    # An approved audit is a new evidence-backed revision, not a fill-empty
    # operation. Keeping the old values here made the analyzer report the
    # correct logo/palette while the active brand continued using stale data.
    merged = _merge_brand_analysis(brand, published_analysis, overwrite=True)
    ...
    connection = get_db()
    published_logo = str(published_analysis.get('logo_url') or '').strip()
    published_upload_logo = published_logo if published_logo.startswith('/static/') else None
    try:
        with connection.cursor() as cursor:
            expected_job_id = str(review_pack.get('job_id') or '')
            cursor.execute(
                "SELECT analysis_metadata FROM cx_clients WHERE id = %s AND crm_client_id = %s FOR UPDATE",
                (brand_id, client_id),
            )
            locked = cursor.fetchone()
            if not locked:
                raise LookupError('Marca não encontrada durante publicação automática.')
            locked_metadata = locked.get('analysis_metadata') if isinstance(locked, dict) else locked[0]
            if isinstance(locked_metadata, str):
                locked_metadata = json.loads(locked_metadata or '{}')
            locked_pack = dict((locked_metadata or {}).get('review_pack') or {})
            if expected_job_id and locked_pack.get('job_id') != expected_job_id:
                raise ValueError('A proposta foi substituída por uma auditoria mais recente.')
            cursor.execute(
                """UPDATE cx_clients
                      SET name = CASE WHEN COALESCE(brand_profile->>'name_autogenerated', 'false') = 'true'
                                      THEN COALESCE(NULLIF(%s, ''), name) ELSE name END,
                          sector = COALESCE(NULLIF(%s, ''), sector), website_url = COALESCE(%s, website_url),
                          logo_url = COALESCE(NULLIF(%s, ''), logo_url),
                          logo_upload_path = COALESCE(NULLIF(%s, ''), logo_upload_path),
                          primary_color = COALESCE(NULLIF(%s, ''), primary_color),
                          secondary_color = COALESCE(NULLIF(%s, ''), secondary_color),
                          tone_of_voice = COALESCE(NULLIF(%s, ''), tone_of_voice),
                          brand_profile = %s::jsonb, analysis_metadata = %s::jsonb
                    WHERE id = %s AND crm_client_id = %s RETURNING id""",
                (published_analysis.get('name'), published_analysis.get('sector'), published_analysis.get('website_url'), published_logo,
                 published_upload_logo,
            ...
            if not cursor.fetchone():
                raise LookupError('Marca não encontrada durante publicação automática.')
            if published_logo:
                # Website images enter the library as candidates. Only the
                # logo selected by the visual resolver becomes approved and
                # primary; competing website marks are rejected so partner or
                # platform logos cannot become the avatar later.
                cursor.execute(
                    """UPDATE cx_client_brand_assets
                          SET status = CASE
                                WHEN asset_path = %s OR source_url = %s THEN 'approved'
                                WHEN role = 'logo' AND source_kind = 'website' THEN 'rejected'
                                ELSE status END,
                              is_primary = CASE
    ...
    try:
        ...
        _save_brand_campaigns(
            client_id, brand_id, str(review_pack.get('job_id') or '') or None,
            list(merged['profile'].get('campaigns') or []),
        )
...
def _reevaluate_saved_brand_audit(client_id: int, user_id: int, brand_id: int, *, apply=True) -> dict:
    """Re-score saved evidence without making or charging a provider call."""
    ...
    with connection.cursor() as cursor:
        cursor.execute(
            '''SELECT job_id, analysis_mode, collected_data, reviews
                 FROM cadu_workspace_brand_audit_runs
                WHERE client_id = %s AND brand_id = %s
                  AND collected_data ? 'analysis_result'
                ORDER BY created_at DESC LIMIT 1''',
            (client_id, brand_id),
        )
        row = cursor.fetchone()
    if not row:
        raise ValueError('Nenhuma auditoria salva possui evidências reutilizáveis.')
    collected = row.get('collected_data') if isinstance(row.get('collected_data'), dict) else {}
    analysis = collected.get('analysis_result') if isinstance(collected.get('analysis_result'), dict) else {}
    if not analysis:
        raise ValueError('A auditoria salva não contém uma proposta reutilizável.')
    metadata = analysis.get('analysis_metadata') if isinstance(analysis.get('analysis_metadata'), dict) else {}
    reviews = row.get('reviews') if isinstance(row.get('reviews'), list) else []
    central = next((item for item in reviews if item.get('id') == 'revisor_central'), {})
    proposal = _brand_analysis_proposal(analysis)
    decision = _automatic_brand_decision(proposal, metadata, central, row.get('analysis_mode') or 'complete')
    decision.update({
        'reevaluated': True, 'source_job_id': str(row.get('job_id') or ''),
        'reevaluated_at': _utc_timestamp(),
    })
    if apply and decision['approved']:
        _auto_apply_brand_analysis(client_id, user_id, brand_id, brand, proposal, decision)
        status = 'approved'
    elif apply:
        current_metadata = dict(brand.get('analysis_metadata') or {})
        current_metadata.update({
            'automatic_decision': decision,
            'score_version': BRAND_ANALYSIS_SCORE_VERSION,
        })
        with connection.cursor() as cursor:
            cursor.execute(
                '''UPDATE cx_clients SET analysis_metadata = %s::jsonb
                    WHERE id = %s AND crm_client_id = %s''',
                (json.dumps(current_metadata), brand_id, client_id),
            )
        connection.commit()
        status = 'insufficient_evidence'
    else:
        status = 'simulation'
    return {'status': status, 'decision': decision, 'job_id': str(row.get('job_id') or '')}


def _ensure_brand_audit_credit(client_id: int, required_credits: int = 1) -> None:
...
def _brand_audit_credit_gate(client_id: int, required_credits: int = 1):
    ...
    try:
        _ensure_brand_audit_credit(client_id, required_credits)
...
def _save_brand_audit_history(client_id: int, brand_id: int, job_id: str, *, analysis_mode='complete', status='queued', input_data=None, analysis=None, reviews=None, costs=None, error='', requested_by=None):
    ...
    collected = {
        'fields': [key for key in ('brand_summary', 'tone_of_voice', 'target_audience', 'products_services', 'differentiators', 'proof_points', 'color_palette', 'social_links', 'product_palettes', 'campaigns') if analysis.get(key) not in (None, '', [], {})],
        ...
        'campaigns': list(analysis.get('campaigns') or [])[:8],
    ...
    connection = get_db()
    # Optional audit persistence must recover from a previously aborted read;
    # it never inherits uncommitted business changes from another stage.
    connection.rollback()
    try:
        with connection.cursor() as cursor:
            cursor.execute(
...
def _save_brand_audit_evidence(client_id: int, brand_id: int, job_id: str, analysis: dict, *, costs=None, decision=''):
    """Persist normalized provenance without coupling publication to the audit UI.

    The compact JSON history remains backwards compatible; these records make
    individual claims, sources, assets and model stages queryable afterwards.
    """
    metadata = dict((analysis or {}).get('analysis_metadata') or {})
    pages = [item for item in metadata.get('evidence_pages') or [] if isinstance(item, dict)]
    ledger = [item for item in (analysis or {}).get('evidence_ledger') or [] if isinstance(item, dict)]
    accepted = [item for item in (analysis or {}).get('asset_candidates') or [] if isinstance(item, dict)]
    rejected = [item for item in (analysis or {}).get('rejected_asset_candidates') or [] if isinstance(item, dict)]
    connection = get_db()
    connection.rollback()
    try:
        with connection.cursor() as cursor:
            ...
            for item in ledger[:24]:
                cursor.execute(
                    '''INSERT INTO cadu_workspace_brand_audit_evidence
                       (job_id, client_id, brand_id, field_name, claim, status, source_url, excerpt, confidence)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)''',
                    (job_id, client_id, brand_id, 'evidence_ledger', str(item.get('claim') or '')[:2000],
                     str(item.get('status') or 'unverified')[:16], str(item.get('source_url') or '')[:4000] or None,
                     str(item.get('excerpt') or '')[:2000] or None, item.get('confidence') or None),
                )
            for asset, outcome in [(item, 'accepted') for item in accepted[:24]] + [(item, 'rejected') for item in rejected[:24]]:
                url = str(asset.get('url') or '')[:4000]
                if not url.startswith(('http://', 'https://')):
                    continue
                cursor.execute(
                    '''INSERT INTO cadu_workspace_brand_audit_assets
                       (job_id, client_id, brand_id, asset_url, asset_kind, decision, ocr_text, relevance, source_url, reason)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                       ON CONFLICT (job_id, asset_url) DO UPDATE SET decision=EXCLUDED.decision, ocr_text=EXCLUDED.ocr_text,
                         relevance=EXCLUDED.relevance, reason=EXCLUDED.reason''',
                    (job_id, client_id, brand_id, url, str(asset.get('kind') or 'reference')[:32], outcome,
                     str(asset.get('ocr_text') or asset.get('ocr') or '')[:4000] or None, asset.get('score') or None,
                     str(asset.get('page_url') or asset.get('source_url') or '')[:4000] or None,
                     str(asset.get('reason') or '')[:2000] or None),
                )
            for stage, detail in dict((costs or {}).get('stages') or {}).items():
                cursor.execute(
                    '''INSERT INTO cadu_workspace_brand_audit_agents
                       (job_id, stage, provider, model, input_tokens, output_tokens, cost_usd, duration_ms)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s)
                       ON CONFLICT (job_id, stage) DO UPDATE SET model=EXCLUDED.model, input_tokens=EXCLUDED.input_tokens,
                         output_tokens=EXCLUDED.output_tokens, cost_usd=EXCLUDED.cost_usd, duration_ms=EXCLUDED.duration_ms''',
                    (job_id, str(stage)[:64], str(detail.get('provider') or '')[:32] or None,
            ...
            if str(decision or '') == 'approved':
                cursor.execute(
                    """UPDATE cadu_workspace_brand_profile_snapshots
                          SET is_current = FALSE, superseded_at = COALESCE(superseded_at, NOW())
                        WHERE brand_id = %s AND is_current AND job_id <> %s""",
                    (brand_id, job_id),
                )
                cursor.execute(
                    """UPDATE cadu_workspace_brand_profile_snapshots
                          SET is_current = TRUE, approved_at = COALESCE(approved_at, NOW()),
                              snapshot_version = GREATEST(snapshot_version,
                                  COALESCE((SELECT identity_revision + 1 FROM cx_clients WHERE id = %s), 1))
                        WHERE id = %s""",
                    (brand_id, snapshot_id),
                )
                cursor.execute(
                    """UPDATE cx_clients
                          SET active_brand_snapshot_id = %s, active_brand_audit_job_id = %s,
                              identity_revision = identity_revision + 1,
                              last_audited_at = NOW(), next_analysis_at = NOW() +
                                  (COALESCE(audit_cadence_days, 60) * INTERVAL '1 day')
                        WHERE id = %s AND crm_client_id = %s""",
                    (snapshot_id, job_id, brand_id, client_id),
                )
...
def _save_brand_campaigns(client_id: int, brand_id: int, job_id: str, campaigns: list[dict]) -> None:
    """Upsert campaign opportunities separately from the active brand profile."""
    connection = get_db()
    connection.rollback()
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """UPDATE cadu_workspace_brand_campaigns
                      SET status = 'superseded', updated_at = NOW()
                    WHERE client_id = %s AND brand_id = %s
                      AND status <> 'project_created' AND audit_job_id IS DISTINCT FROM %s""",
                (client_id, brand_id, job_id),
            )
            for campaign in campaigns[:8]:
                if not isinstance(campaign, dict):
                    continue
                campaign_id = str(campaign.get('id') or '')[:64]
                name = str(campaign.get('name') or '').strip()[:160]
                if not campaign_id or not name:
                    continue
                cursor.execute(
                    '''INSERT INTO cadu_workspace_brand_campaigns
                       (id, client_id, brand_id, audit_job_id, name, campaign_type, objective, audience,
                        channels, rationale, source_url, confidence, status)
                       VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb,%s,%s,%s,%s)
                       ON CONFLICT (id) DO UPDATE SET audit_job_id=EXCLUDED.audit_job_id, name=EXCLUDED.name,
                        campaign_type=EXCLUDED.campaign_type, objective=EXCLUDED.objective, audience=EXCLUDED.audience,
                        channels=EXCLUDED.channels, rationale=EXCLUDED.rationale, source_url=EXCLUDED.source_url,
                        confidence=EXCLUDED.confidence, status=CASE WHEN cadu_workspace_brand_campaigns.status = 'project_created'
                        THEN cadu_workspace_brand_campaigns.status ELSE EXCLUDED.status END, updated_at=NOW()''',
                    (campaign_id, client_id, brand_id, job_id, name, str(campaign.get('type') or 'paid_media')[:24],
                     str(campaign.get('objective') or '')[:4000] or None, str(campaign.get('audience') or '')[:4000] or None,
                     json.dumps(list(campaign.get('channels') or [])[:6]), str(campaign.get('rationale') or '')[:6000] or None,
                     str(campaign.get('source_url') or '')[:4000] or None, campaign.get('confidence') or None,
                     str(campaign.get('status') or 'opportunity')[:24]),
                )
        connection.commit()
    except Exception:
        connection.rollback()
        current_app.logger.exception('Não foi possível registrar campanhas da auditoria %s', job_id)


def _brand_campaigns(client_id: int, brand_id: int, fallback=None) -> list[dict]:
    """Read independent campaign records, keeping old audits usable as fallback."""
    try:
        with get_db().cursor() as cursor:
            cursor.execute(
                '''SELECT id, name, campaign_type, objective, audience, channels, rationale, source_url,
                          confidence, status, project_id
                    FROM cadu_workspace_brand_campaigns
                    WHERE client_id = %s AND brand_id = %s AND status <> 'superseded'
                    ORDER BY updated_at DESC, name ASC LIMIT 12''', (client_id, brand_id),
            )
            rows = cursor.fetchall()
        if rows:
            return [{
                'id': str(row.get('id') or ''), 'name': str(row.get('name') or ''),
                'type': str(row.get('campaign_type') or 'paid_media'),
                'objective': str(row.get('objective') or ''), 'audience': str(row.get('audience') or ''),
                'channels': row.get('channels') if isinstance(row.get('channels'), list) else [],
                'rationale': str(row.get('rationale') or ''), 'source_url': str(row.get('source_url') or ''),
                'confidence': float(row.get('confidence') or 0), 'status': str(row.get('status') or 'opportunity'),
    ...
    try:
        ...
        for row in rows:
            ...
            item['reliability'] = {
                ...
                'successful_calls': max(0, int(reliability.get('successful_calls') or 0)),
                'failed_calls': max(0, int(reliability.get('failed_calls') or 0)),
                'fallback_used': bool(reliability.get('fallback_used')),
                'partial_result': bool(reliability.get('partial_result')),
            }
            ...
            collected.pop('analysis_result', None)
            collected.pop('evidence_pages', None)
            item['collected_data'] = collected
            if total_usd > 0:
...
def _brand_audit_reliability_summary(history: list[dict]) -> dict:
    """Aggregate provider health without treating missing legacy telemetry as failure."""
    completed = [item for item in history if item.get('status') in {'approved', 'pending_approval', 'insufficient_evidence'}]
    measured = [item for item in completed if int((item.get('reliability') or {}).get('provider_calls') or 0) > 0]
    provider_calls = sum(int((item.get('reliability') or {}).get('provider_calls') or 0) for item in measured)
    successful_calls = sum(int((item.get('reliability') or {}).get('successful_calls') or 0) for item in measured)
    failed_calls = sum(int((item.get('reliability') or {}).get('failed_calls') or 0) for item in measured)
    stable_runs = sum(
        1 for item in measured
        if not (item.get('reliability') or {}).get('partial_result') and not (item.get('reliability') or {}).get('failed_calls')
    )
    return {
        'runs': len(completed),
        ...
        'fallback_runs': sum(1 for item in measured if (item.get('reliability') or {}).get('fallback_used')),
        'partial_runs': sum(1 for item in measured if (item.get('reliability') or {}).get('partial_result')),
        'call_success_rate': round(successful_calls * 100 / provider_calls) if provider_calls else None,
        'stable_run_rate': round(stable_runs * 100 / len(measured)) if measured else None,
    }
...
def _start_brand_review_job(client_id: int, user_id: int, brand_id: int, job_id: str, website_url: str, images: list[dict], proposal=None, *, background=True, analysis_mode='complete', social_links=None, additional_sources=None, excluded_sources=None, existing_asset_ids=None):
        ...
        with app.app_context():
            try:
                ...
                if existing_asset_ids is None and not restored_images:
                    # Legacy callers did not submit a selection. Reuse a
                    # balanced visual set instead of only one primary logo.
                    selected_assets = sorted((
                        asset for asset in stored_brand.get('assets', [])
                        if str(asset.get('status') or '').lower() == 'approved'
                        and (asset.get('asset_path') or asset.get('source_url'))
                    ), key=lambda item: (
                        not bool(item.get('is_primary')),
                        str(item.get('role') or '') == 'logo',
                        -float(item.get('score') or 0),
                    ))[:12]
                else:
                    selected_assets.sort(key=lambda item: (
                        not bool(item.get('is_primary')),
                        str(item.get('role') or '') == 'logo',
                        -float(item.get('score') or 0),
                    ))
                # Collapse exact/import duplicates and keep at most two logo
                # variants so campaign, photography and application examples
                # reach the visual model as well.
                curated_assets, seen_assets, logo_count = [], set(), 0
                for asset in selected_assets:
                    asset_meta = asset.get('metadata') if isinstance(asset.get('metadata'), dict) else {}
                    fingerprint = str(
                        asset_meta.get('perceptual_hash') or asset_meta.get('sha256')
                        or asset.get('asset_path') or str(asset.get('source_url') or '').split('?', 1)[0]
                        or asset.get('id')
                    )
                    if fingerprint in seen_assets:
                        continue
                    is_logo = str(asset.get('role') or '').lower() == 'logo'
                    if is_logo and logo_count >= 2:
                        continue
                    seen_assets.add(fingerprint)
                    logo_count += 1 if is_logo else 0
                    curated_assets.append(asset)
                selected_assets = curated_assets
                brand_host = (urlparse(str(stored_brand.get('website_url') or '')).hostname or '').lower()
                max_selected_assets = 12 if analysis_mode == 'deep' else 8
                for asset in selected_assets[:max_selected_assets]:
                    try:
                        ...
                        if not source_url.startswith(('http://', 'https://')) or not brand_host or not (
                            source_host == brand_host or source_host.endswith('.' + brand_host)
                        ):
                            continue
                        from .brand_site_inspector import _request
                        response, final_url = _request(
                            source_url, accept='image/*', max_bytes=8 * 1024 * 1024,
                        )
                        if response.status_code >= 400:
                            response.close()
                            continue
                        final_host = (urlparse(str(final_url or '')).hostname or '').lower()
                        if not (final_host == brand_host or final_host.endswith('.' + brand_host)):
                            response.close()
                            continue
                        content = b''.join(response.iter_content(64 * 1024))
                        content_type = str(response.headers.get('Content-Type') or asset.get('mime_type') or 'image/png').split(';', 1)[0]
                        response.close()
                        if not content or not content_type.startswith('image/'):
                            continue
                        restored_images.append(FileStorage(
                            stream=BytesIO(content), filename=f'brand-asset-{asset.get("id")}.img',
                        ...
                        current_app.logger.warning(
                            'Ativo remoto %s da marca %s não pôde ser baixado para a auditoria.',
                            asset.get('id'), brand_id,
                        )
                ...
                token_usage = {
                    ...
                    'provider_tokens': 0, 'typesafe_tokens': 0, 'calls': 0, 'stages': {},
                }
                ...
                def bill(stage, provider_result, model):
                    ...
                    token_usage['stages'][stage] = {
                        'model': model,
                        'provider': 'perplexity' if 'perplexity' in str(model).lower() else 'openai',
                        'input_tokens': prompt_tokens,
                        'output_tokens': completion_tokens,
                        'cost_usd': (usage or {}).get('cost') or (usage or {}).get('total_cost'),
                    }
                ...
                if isinstance(review_proposal, dict) and review_proposal:
                    ...
                    analysis = service.analyze_brand(
                        ...
                        brand_name=str(stored_brand.get('name') or ''), preflight=True,
                        has_trusted_brand_assets=bool(selected_assets),
                    )
                    ...
                    selected_official_logo = next((
                        asset for asset in selected_assets
                        if bool(asset.get('is_primary'))
                        and isinstance(asset.get('metadata'), dict)
                        and asset['metadata'].get('user_selected')
                        and str(asset.get('role') or '').lower() == 'logo'
                        and asset.get('asset_path')
                    ), None)
                    if selected_official_logo:
                        selected_path = CreativeAssetStorage().absolute_public_path(
                            selected_official_logo.get('asset_path'),
                        )
                        if selected_path:
                            analysis['logo_url'] = str(selected_official_logo['asset_path'])
                ...
                _save_brand_review_job(client_id, brand_id, job_id,
                    ...
                    message='Duas revisões e a consolidação central estão prontas para decisão.', error='',
                ...
                failed_costs = locals().get('token_usage', {})
                if isinstance(preflight_report, dict):
                    ...
                    if preflight_report.get('source') == 'firecrawl':
                        try:
                            credits.charge_firecrawl(
                                actor=actor, idempotency_key=f'workspace-brand:{job_id}:firecrawl-preflight',
                                operation='scrape', pages=1, app='Auditoria de marca',
                                stage='validacao_previa',
                                metadata={'brand_id': brand_id, 'job_id': job_id, 'source': 'workspace',
                                          'preflight_decision': preflight_report.get('decision')},
                            )
                        except Exception:
                            current_app.logger.exception('Não foi possível registrar a leitura inicial da auditoria %s', job_id)
                _save_brand_review_job(client_id, brand_id, job_id,
                ...
                _save_brand_audit_history(client_id, brand_id, job_id, analysis_mode=analysis_mode, status='failed',
                                          input_data={'website_url': website_url, 'social_links': list(social_links or []),
                                                      ...
                                                      'existing_asset_ids': list(existing_asset_ids or []),
                                                      'preflight': preflight_report if isinstance(preflight_report, dict) else {}},
                                          costs=failed_costs, error=str(exc),
                                          requested_by=user_id)
                try:
                    from .. import db
                    from ..services.cadu_product_emails import send_brand_audit_ready
    ...
    if not current_app.config.get('CADU_BRAND_AUDIT_WORKER_ENABLED', False):
        raise RuntimeError('A fila durável de auditoria de marca está desabilitada.')
...
def _run_brand_module_review_job(client_id, user_id, brand_id, job_id, module_id, analysis):
    """Run a saved-evidence opinion inside the supervised audit worker."""
    from ..creative_modeling_service import CreativeModelingService
    from ..cadu_credit_connector import CaduCreditConnector, CreditActor
    ...
    order = {'evidencias': 0, 'ampliacao': 1, 'revisor_central': 2}
    ...
    return _save_brand_review_job(
        client_id, brand_id, job_id, status='pending_approval', stage='complete',
        index=4, total=4, message='Parecer atualizado. Revise a proposta antes de aplicar.',
        error='', reviews=reviews, analysis_metadata={
            'central_review': central_review, 'ready_for_approval': central_ready,
        },
    )
...
def _workspace_brand_catalog_metrics(client_id: int) -> dict[str, dict]:
    """Return operational counters for the brand catalog.

    Metrics are derived from the canonical project-brand links instead of
    duplicating counters on the brand record. Missing rollout tables degrade
    to zeroes so the catalog remains usable during migrations.
    """
    try:
        with get_db().cursor() as cursor:
            cursor.execute(
                """WITH linked AS (
                     ...
                     SELECT linked.brand_ref,
                            COUNT(DISTINCT p.id) FILTER (WHERE p.status = 'ativo') AS active_projects,
                            COALESCE(MAX(cc.total), 0) AS conversations,
                            COALESCE(SUM(fc.total), 0) AS files,
                            COALESCE(jsonb_object_agg(fc.category, fc.total)
                                     FILTER (WHERE fc.category IS NOT NULL), '{}'::jsonb) AS file_categories
                       FROM linked
                  LEFT JOIN cadu_ci_projetos p
                         ON linked.project_ref = 'ci:' || p.id::text
                        AND p.id_cliente = %s
                  LEFT JOIN conversation_counts cc ON cc.projeto_id = p.id
                  LEFT JOIN file_counts fc ON fc.project_id = p.id::text
                   GROUP BY linked.brand_ref""",
                (client_id, client_id, client_id, client_id),
            )
            result = {}
            for row in cursor.fetchall():
                categories = row.get('file_categories') or {}
                if isinstance(categories, str):
                    try:
                        categories = json.loads(categories)
                    except (TypeError, ValueError):
                        categories = {}
                result[str(row.get('brand_ref') or '').removeprefix('studio:')] = {
                    'activeProjects': int(row.get('active_projects') or 0),
                    'conversationCount': int(row.get('conversations') or 0),
                    'fileCount': int(row.get('files') or 0),
                    'fileCategories': {str(key): int(value or 0) for key, value in categories.items()},
                }
            return result
    except Exception:
        current_app.logger.warning('Não foi possível calcular métricas do catálogo de marcas', exc_info=True)
        return {}
...
def brands():
    ...
    catalog_error = ''
    try:
        catalog_records = _workspace_brands(client_id, query, raise_on_error=True)
    except Exception:
...
def inspect_brand_site_route():
    ...
    token = URLSafeTimedSerializer(current_app.secret_key, salt='workspace-brand-site-inspection').dumps({
        ...
        'suggested_name': result.get('suggested_name') or '',
        'suggested_sector': result.get('suggested_sector') or '',
        'logo_candidates': result.get('logo_candidates') or [],
    })
...
def create_brand():
    ...
    if inspection_token:
        ...
        discovered_logos = {
            _normalized_website_url(item.get('url') or '')
            for item in (inspected.get('logo_candidates') or [])
            if isinstance(item, dict) and item.get('url')
        }
        if posted_logo and posted_logo != inspected_logo and posted_logo not in discovered_logos:
            abort(400, description='A logo mudou depois da inspeção. Valide os links novamente.')
        submitted_logo_url = posted_logo or inspected_logo or _normalized_website_url(
            inspected.get('suggested_logo_url') or '',
        )
        if inspected.get('suggested_name') and (
                data.get('name_autogenerated')
                or data.get('name') == str(inspected.get('suggested_name')).strip()):
            data['name'] = ' '.join(str(inspected['suggested_name']).split())[:150]
            data['name_autogenerated'] = True
            data['profile']['name_autogenerated'] = True
        if not data.get('sector') and inspected.get('suggested_sector'):
            data['sector'] = ' '.join(str(inspected['suggested_sector']).split())[:80]
    ...
    if files:
        try:
            from ..creative_modeling_service import CreativeModelingService
            try:
                primary_logo_index = max(0, min(7, int(request.form.get('primary_logo_index') or 0)))
            except (TypeError, ValueError):
                primary_logo_index = 0
            CreativeModelingService().upload_client_brand_assets(
                brand_id, files, request.form.get('primary_logo') == 'true', 'reference',
                primary_logo_index=primary_logo_index,
            )
        except ValueError as exc:
    ...
    if submitted_logo_url:
        try:
            from ..creative_modeling_service import CreativeModelingService
            saved_logo = CreativeModelingService().import_website_brand_assets(brand_id, [{
                'url': submitted_logo_url,
                'page_url': data.get('website_url') or '',
                'kind': 'logo',
                'category': 'Logo oficial selecionada',
                'reason': 'Selecionada pela equipe entre logos PNG, JPG ou WebP validadas no site oficial.',
                'score': 100,
                'is_primary': True,
                'user_selected': True,
            }])
            if not saved_logo or not saved_logo[0].get('asset_path'):
                raise ValueError('A logo foi validada, mas não foi possível armazenar a cópia usada pela análise.')
        except ValueError as exc:
            current_app.logger.exception('Marca %s criada sem persistir a logo oficial selecionada', brand_id)
            abort(503, description=str(exc))
        except Exception:
            current_app.logger.exception('Marca %s criada sem persistir a logo oficial selecionada', brand_id)
            abort(503, description='A logo escolhida não pôde ser armazenada. Tente buscar outra opção.')
    ...
    has_audit_source = bool(data.get('website_url') or files or submitted_logo_url)
    return redirect(url_for('cadu_workspace.brand_detail', brand_id=brand_id,
                            audit='start' if request.form.get('analyze') == 'true' and has_audit_source else None), code=303)
...
def projects():
    ...
    status = status if status in {'ativos', 'arquivados', 'todos'} else 'ativos'
    if request.args.get('legacy') == '1':
        return redirect(url_for('cadu_workspace.clean_projects', q=query, status=status), code=308)
    catalog_error = ''
    try:
        catalog_records = _workspace_projects(client_id, query, status=status, raise_on_error=True)
    except Exception:
...
def create_campaign_project(brand_id, campaign_id):
    ...
    if not campaign:
        abort(404)
    name = str(campaign.get('name') or '').strip()[:150]
    if not name:
        abort(409, description='Esta sugestão não possui um nome de campanha utilizável.')
    objective = str(campaign.get('objective') or '').strip()
    audience = str(campaign.get('audience') or '').strip()
    channels = ', '.join(str(item) for item in campaign.get('channels') or [])
    rationale = str(campaign.get('rationale') or '').strip()
    instructions = '\n'.join(filter(None, [
        f'# {name}',
        ...
        'Use a identidade aprovada da marca como restrição. Esta é uma oportunidade de campanha, não uma alteração da identidade permanente.',
    ]))[:12000]
    project_id = str(uuid4())
    connection = get_db()
    try:
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                (project_id, str(campaign_id), client_id, brand_id),
            )
        connection.commit()
    except Exception:
        connection.rollback()
...
def brand_detail(brand_id):
    ...
    if not brand:
        abort(404)
    # Normalize partially enriched records before creating the React contract.
    # Missing optional projections must not turn the detail page into a 500.
    ...
    if request.args.get('legacy') != '1':
        # The shared dock needs the complete catalogs; load them once and pass
        # them through rather than asking the dock helper to query them again.
        ...
        brand_links = {
            ...
            'createCampaignProjectBase': f'{brand_base}/campanhas/__CAMPAIGN_ID__/projeto',
        }
        brand_data = {
            ...
            'profile': {
                ...
                'campaigns': campaigns,
                'productsServices': profile.get('products_services') if isinstance(profile.get('products_services'), list) else [],
                'differentiators': profile.get('differentiators') if isinstance(profile.get('differentiators'), list) else [],
                'proofPoints': profile.get('proof_points') if isinstance(profile.get('proof_points'), list) else [],
                'adSegments': profile.get('ad_segments') if isinstance(profile.get('ad_segments'), list) else [],
                'visualMotifs': profile.get('visual_motifs') if isinstance(profile.get('visual_motifs'), list) else [],
                'mandatoryElements': profile.get('mandatory_elements') if isinstance(profile.get('mandatory_elements'), list) else [],
                'forbiddenElements': profile.get('forbidden_elements') if isinstance(profile.get('forbidden_elements'), list) else [],
                'contacts': profile.get('contacts') if isinstance(profile.get('contacts'), list) else [],
                'addresses': profile.get('addresses') if isinstance(profile.get('addresses'), list) else [],
                'digitalPolicies': profile.get('digital_policies') if isinstance(profile.get('digital_policies'), list) else [],
                'competitors': profile.get('competitors') if isinstance(profile.get('competitors'), list) else [],
                'evidenceLedger': profile.get('evidence_ledger') if isinstance(profile.get('evidence_ledger'), list) else [],
                'fieldProvenance': profile.get('field_provenance') if isinstance(profile.get('field_provenance'), dict) else {},
            },
            'readiness': brand.get('readiness') or {'score': 0, 'missing': []}, 'reviewPack': review_pack,
            'auditHistory': audit_history,
            'reliabilitySummary': _brand_audit_reliability_summary(audit_history),
            'analysisMetadata': {
                ...
                'screenshot': str((brand.get('analysis_metadata') or {}).get('screenshot') or ''),
                'screenshotOcr': str((brand.get('analysis_metadata') or {}).get('screenshot_ocr') or ''),
            ...
            'assets': [{
                ...
                'reusable': bool(item.get('display_url')) and not bool(item.get('missing_file')) and str(item.get('status') or '').lower() == 'approved',
        ...
        dock_items = _workspace_common_dock_items(
            client_id, int(session.get('user_id') or 0),
...
def brand_context_api(brand_id):
    ...
    metadata = dict(brand.get('analysis_metadata') or {})
    context = {
        'brand_id': brand_id, 'brand_ref': f'studio:{brand_id}', 'name': brand.get('name'),
        'website_url': brand.get('website_url'), 'logo_url': brand.get('display_logo') or brand.get('logo_url'),
        ...
        'contacts': profile.get('contacts') or [], 'addresses': profile.get('addresses') or [],
        'policies': profile.get('digital_policies') or [], 'campaigns': _brand_campaigns(client_id, brand_id, profile.get('campaigns') or []),
        ...
        'detail_url': url_for('cadu_workspace.clean_brand_detail', brand_id=brand_id),
    }
...
def audit_brand(brand_id):
    ...
    _workspace_team_admin()
    if not current_app.config.get('CADU_BRAND_AUDIT_WORKER_ENABLED', False):
        abort(503, description='A fila de auditorias está indisponível. Ative o worker supervisionado e tente novamente.')
    client_id = int(session.get('cliente_id') or 0)
    brand = _workspace_brand(client_id, brand_id)
    if not brand:
        abort(404)
    ...
    credit_response = _brand_audit_credit_gate(client_id, 150000 if analysis_mode == 'deep' else 75000)
    if credit_response is not None:
        return credit_response
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            if isinstance(locked_metadata, str):
                locked_metadata = json.loads(locked_metadata or '{}')
            active_pack = dict((locked_metadata or {}).get('review_pack') or {})
            if active_pack.get('status') in {'queued', 'running'}:
                abort(409, description='Esta marca já está sendo processada. Aguarde a conclusão antes de iniciar outra análise.')
            # Avoid overwriting other metadata changes made after the initial
            # request read while the user was selecting files.
            metadata = dict(locked_metadata or {})
            metadata['review_pack'] = {
                'job_id': job_id, 'status': 'queued', 'stage': 'queued', 'index': 0, 'total': 5,
                'message': 'A auditoria entrou na fila.', 'error': '', 'created_at': _utc_timestamp(),
                'input': {'website_url': website_url, 'has_images': bool(image_payload),
                          'has_logo_upload': bool(logo_uploads), 'existing_asset_ids': list(existing_asset_ids or []),
                          'include_project_sources': request.form.get('include_project_sources') == 'true',
                          'analysis_mode': analysis_mode, 'social_links': social_links,
                          'additional_sources': additional_sources, 'excluded_sources': excluded_sources,
                          'pipeline_version': BRAND_ANALYSIS_PIPELINE_VERSION},
    ...
    if logo_uploads:
        try:
            from ..creative_modeling_service import CreativeModelingService
            # Audit uploads remain candidates until the proposal is approved.
            CreativeModelingService().upload_client_brand_assets(
                brand_id, logo_uploads, False, 'reference',
            )
            logo_uploads[0].stream.seek(0)
        except Exception:
            current_app.logger.exception('Não foi possível preservar o logo de referência da auditoria %s', brand_id)
    if reference_images:
        try:
            from ..creative_modeling_service import CreativeModelingService
    ...
    try:
        ...
        current_app.logger.exception('Não foi possível enfileirar a auditoria da marca %s', brand_id)
        _save_brand_review_job(client_id, brand_id, job_id, status='failed', stage='failed',
                               message='A auditoria não entrou na fila.', error='Falha ao gravar o job durável.')
        if request.accept_mimetypes.best == 'application/json':
            return jsonify({'ok': False, 'error': 'Não foi possível enfileirar a auditoria agora.'}), 503
        abort(503, description='Não foi possível enfileirar a auditoria agora.')
    if request.accept_mimetypes.best == 'application/json':
        return jsonify({
...
def brand_audit_status(brand_id):
    ...
    if pack.get('job_id') and _brand_review_is_stale(pack):
        try:
            ...
            from .brand_audit_jobs import fail_stale
            fail_stale(str(pack['job_id']))
            pack.update({
...
def retry_brand_audit(brand_id):
    ...
    include_project_sources = (
        request.form.get('include_project_sources') == 'true'
        if request.form.get('include_project_sources_present') == 'true'
        else bool(previous_input.get('include_project_sources'))
    )
    metadata['review_pack'] = {
        'job_id': job_id, 'status': 'queued', 'stage': 'queued', 'index': 0, 'total': 5,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            metadata['review_pack'] = {
                'job_id': job_id, 'status': 'queued', 'stage': 'queued', 'index': 0, 'total': 5,
                'message': 'A nova análise entrou na fila. A identidade atual será preservada até sua aprovação.',
                'error': '', 'created_at': _utc_timestamp(),
                'input': {'website_url': website_url, 'has_images': bool(image_payload),
                          'has_logo_upload': bool(logo_uploads), 'preserves_logo': True,
                          'existing_asset_ids': list(existing_asset_ids or []),
                          'include_project_sources': include_project_sources,
                          'analysis_mode': analysis_mode, 'social_links': social_links,
                          'additional_sources': additional_sources, 'excluded_sources': excluded_sources,
                          'pipeline_version': BRAND_ANALYSIS_PIPELINE_VERSION},
                'analysis': checkpoint, 'reviews': [],
            }
            cursor.execute(
                """UPDATE cx_clients SET analysis_metadata = %s::jsonb
                     WHERE id = %s AND crm_client_id = %s RETURNING id""",
    ...
    if logo_uploads:
        try:
            from ..creative_modeling_service import CreativeModelingService
            CreativeModelingService().upload_client_brand_assets(brand_id, logo_uploads, False, 'reference')
            logo_uploads[0].stream.seek(0)
        except Exception:
            current_app.logger.exception('Não foi possível preservar o logo de referência da auditoria %s', brand_id)
    if reference_images:
    ...
    try:
        ...
        current_app.logger.exception('Não foi possível enfileirar a nova auditoria da marca %s', brand_id)
        _save_brand_review_job(client_id, brand_id, job_id, status='failed', stage='failed',
                               message='A auditoria não entrou na fila.', error='Falha ao gravar o job durável.')
        if request.accept_mimetypes.best == 'application/json':
            return jsonify({'ok': False, 'error': 'Não foi possível enfileirar a auditoria agora.'}), 503
...
def refresh_brand_audit_module(brand_id, module_id):
    ...
    if module_id not in {'evidencias', 'ampliacao', 'revisor_central'}:
        abort(404)
    if not _workspace_api_csrf():
        abort(403, description='Atualize a página e tente novamente.')
    _workspace_team_admin()
    ...
    if not current_app.config.get('CADU_BRAND_AUDIT_WORKER_ENABLED', False):
        _save_brand_review_job(client_id, brand_id, job_id, status='failed', stage='failed',
                               message='A fila de auditorias está desabilitada.',
                               error='Ative o worker supervisionado de auditoria para continuar.')
        abort(503, description='A fila de auditorias está indisponível. Tente novamente mais tarde.')
    try:
        from .brand_audit_jobs import enqueue
        enqueue({
            'job_id': job_id, 'client_id': client_id, 'user_id': user_id,
            'brand_id': brand_id, 'job_type': 'module_review', 'module_id': module_id,
            'analysis': analysis, 'images': [], 'analysis_mode': 'complete',
            'request_reason': 'module_refresh',
        })
    except Exception:
        current_app.logger.exception('Não foi possível enfileirar o parecer %s da marca %s', module_id, brand_id)
        _save_brand_review_job(client_id, brand_id, job_id, status='failed', stage='failed',
                               message='O parecer não entrou na fila.', error='Falha ao gravar o job durável.')
        abort(503, description='Não foi possível enfileirar este parecer agora.')
    return redirect(url_for('cadu_workspace.brand_detail', brand_id=brand_id, audit='queued'), code=303)
...
def reevaluate_brand_audit(brand_id):
    ...
    client_id = int(session.get('cliente_id') or 0)
    try:
        result = _reevaluate_saved_brand_audit(
            client_id, int(session.get('user_id') or 0), brand_id,
            apply=request.form.get('simulation') != 'true',
        )
    except LookupError:
        abort(404)
    except ValueError as exc:
...
def approve_brand_reviews(brand_id):
    ...
    blocked_fields = _normalized_blocked_fields(
        ((brand.get('analysis_metadata') or {}).get('automatic_decision') or {}).get('blocked_fields')
        or central_review.get('blocked_fields')
    )
    selectable = {
    ...
    requested = {value for value in request.form.getlist('approved_fields') if value in selectable}
    central_ready = str(central_review.get('status') or '') == 'ready' and not list(central_review.get('blocked_fields') or [])
    if not requested and not central_ready:
        abort(409, description='Selecione explicitamente os campos comprovados ou refaça a consolidação central.')
    approved_fields = requested or selectable
    ...
    review_pack.update({
        ...
        'deferred_fields': sorted(selectable - approved_fields),
        'approval_mode': 'human_field_selection' if requested else 'human_all_fields',
    ...
    try:
        with connection.cursor() as cursor:
            ...
            if isinstance(locked_metadata, str):
                locked_metadata = json.loads(locked_metadata or '{}')
            locked_pack = dict((locked_metadata or {}).get('review_pack') or {})
            if locked_pack.get('job_id') != pack.get('job_id'):
                abort(409, description='A proposta foi substituída. Atualize a página antes de aprovar.')
            cursor.execute(
                """UPDATE cx_clients
                      SET name = CASE WHEN COALESCE(brand_profile->>'name_autogenerated', 'false') = 'true'
                                      THEN COALESCE(NULLIF(%s, ''), name) ELSE name END,
            ...
            if not cursor.fetchone():
                abort(404)
            if 'logo_url' in approved_fields and approved_analysis.get('logo_url'):
                selected_logo = str(approved_analysis['logo_url'])
                cursor.execute(
                    """UPDATE cx_client_brand_assets SET is_primary = FALSE
                        WHERE client_id = %s AND role = 'logo'
                          AND EXISTS (SELECT 1 FROM cx_client_brand_assets
                                       WHERE client_id = %s AND (asset_path = %s OR source_url = %s))""",
                    (brand_id, brand_id, selected_logo, selected_logo),
                )
                cursor.execute(
                    """UPDATE cx_client_brand_assets
                          SET role = 'logo', status = 'approved', is_primary = TRUE
                        WHERE client_id = %s AND (asset_path = %s OR source_url = %s)""",
                    (brand_id, selected_logo, selected_logo),
                )
            ...
            if job_id:
                ...
                cursor.execute(
                    """UPDATE cadu_workspace_brand_profile_snapshots
                          SET decision = 'approved', is_current = TRUE,
                              approved_by = %s, approved_at = NOW(), superseded_at = NULL
                        WHERE job_id = %s AND client_id = %s AND brand_id = %s
                        RETURNING id""",
                    (int(session.get('user_id') or 0), job_id, client_id, brand_id),
                )
                active_snapshot = cursor.fetchone()
                if active_snapshot:
                    snapshot_id = active_snapshot.get('id') if isinstance(active_snapshot, dict) else active_snapshot[0]
                    cursor.execute(
                        """UPDATE cx_clients
                              SET active_brand_snapshot_id = %s, active_brand_audit_job_id = %s,
                                  identity_revision = identity_revision + 1,
    ...
    try:
        ...
        _sync_approved_brand_to_projects(
            client_id, int(session.get('user_id') or 0), brand_id,
            {**brand, 'brand_profile': merged['profile'], 'analysis_metadata': metadata}, approved_analysis,
        )
    except Exception:
    ...
    try:
        _send_brand_approval_email(
            {**brand, 'name': approved_analysis.get('name') or brand.get('name'), 'analysis_metadata': metadata},
    ...
    try:
        ...
        seed_brand = {
            **brand,
            'name': approved_analysis.get('name') if (brand.get('brand_profile') or {}).get('name_autogenerated') else brand.get('name'),
            'sector': approved_analysis.get('sector') or brand.get('sector'),
            'primary_color': approved_analysis.get('primary_color') or brand.get('primary_color'),
            'secondary_color': approved_analysis.get('secondary_color') or brand.get('secondary_color'),
        }
