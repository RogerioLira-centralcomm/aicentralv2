...
def _audiencia_tem_precificacao(data):
    """True quando o payload pede recálculo completo (plataforma + KPI + val. tabela + invest ou volume)."""
    if not isinstance(data, dict):
        return False
    plat = (data.get('audiencia_calculo_plataforma') or data.get('plataforma') or '')
    if isinstance(plat, str):
        plat = plat.strip()
    kpi = (data.get('audiencia_calculo_kpi') or data.get('objetivo_kpi') or '')
    if isinstance(kpi, str):
        kpi = kpi.strip()
    id_kpi = data.get('id_audiencia_calculo_kpi') or data.get('id_objetivo_kpi')
    ...
    has_inv = inv > 0
    has_vol = vol > 0
    return (has_inv and not has_vol) or (has_vol and not has_inv) or (has_inv and has_vol)
...
def _recalcular_e_montar_breakdown(cotacao, data):
    ...
    Retorna `(payload_extra, error_response, status)`:
      - payload_extra: dict com perc_*, val_*, valor_unitario_negociado, volume_contratado,
        investimento_bruto e investimento_liquido recalculados.
...
def cotacao_editar(cotacao_id):
    try:
        ...
        def _resumo_metricas_cotacao():
            ...
            def _add_plataforma(nome):
                nome = (nome or '').strip()
                if nome and nome not in plataformas:
                    plataformas.append(nome)

            _add_plataforma(cotacao.get('plataforma_campanha'))

            for linha in linhas:
                if linha.get('is_header') or linha.get('is_subtotal'):
                    continue
                try:
                    valor_liquido += float(linha.get('investimento_liquido') or 0)
                except (TypeError, ValueError):
                    pass
                _add_plataforma(linha.get('plataforma'))

            for aud in audiencias:
                bruto = aud.get('investimento_bruto') or aud.get('investimento_sugerido')
                liquido = aud.get('investimento_liquido') or bruto
                try:
                    valor_liquido += float(liquido or 0)
                except (TypeError, ValueError):
                    pass
                _add_plataforma(aud.get('audiencia_calculo_plataforma') or aud.get('audiencia_categoria'))

            plataforma_txt = ', '.join(plataformas[:3])
            if len(plataformas) > 3:
                plataforma_txt += f' +{len(plataformas) - 3}'
        ...
        if request.method == 'POST':
            nome_campanha = request.form.get('nome_campanha', '').strip()
            periodo_inicio = request.form.get('periodo_inicio', '').strip()
            valor_total_str = request.form.get('valor_total_proposta') or request.form.get('valor_total_proposta_display')
            ...
            if not nome_campanha or not periodo_inicio:
                ...
                return render_template(
                    ...
                    agencias_opts=db.obter_aux_agencia(),
                    vendedores_cc=db.obter_vendedores_centralcomm(),
                    resumo_metricas=_resumo_metricas_cotacao(),
                    estados=db.obter_estados(),
                )
...
def api_atualizar_cotacao_teste(cotacao_id):
    try:
        ...
        if update_data.get('status') == 'Aprovada' and cotacao.get('status') != 'Aprovada':
            ...
            linhas = db.obter_linhas_cotacao(cotacao_id)
            linhas_com_valor = [
                l for l in (linhas or [])
                if not l.get('is_subtotal') and not l.get('is_header')
                and (l.get('investimento_bruto') or 0) > 0
            ]
            if not linhas_com_valor:
                return jsonify({
                    'success': False,
                    'message': 'A cotação precisa de pelo menos um item de proposta com valor para ser aprovada.',
                }), 400
...
def criar_audiencia_cotacao_api_teste():
    try:
        ...
        if _audiencia_tem_precificacao(data):
            ...
            audiencia_id = db.adicionar_audiencia_cotacao(
                ...
                audiencia_subcategoria=data.get('audiencia_subcategoria'),
                cpm_estimado=None,
                investimento_sugerido=None,
                impressoes_estimadas=None,
                incluido_proposta=data.get('incluido_proposta', True),
                perc_margem_cc=None,
                audiencia_calculo_plataforma=aud_plat or None,
                audiencia_calculo_kpi=None,
                ...
                val_tech_fee=None,
                val_com_vendas=None,
                val_pl_incentivos=None,
                val_impostos=None,
                valor_unitario_tabela=None,
                valor_unitario=None,
                valor_unitario_negociado=None,
                investimento_bruto=None,
                investimento_liquido=None,
