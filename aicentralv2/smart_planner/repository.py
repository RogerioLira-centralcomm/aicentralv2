...
def create_session(user: dict, plan_mode: str, seed: dict | None = None) -> dict:
    token = secrets.token_hex(32)
    seed = seed or {}
    canais = as_list(seed.get("canais"))
    dados = {
        "plan_mode": plan_mode,
        "cliente": text(seed.get("cliente")),
    ...
    cliente = text(seed.get("cliente")) or None
    conn = get_db()
    with conn.cursor() as cur:
        try:
            cur.execute(
                ...
                INSERT INTO cadu_smart_planner_sessions
                    (session_token, user_id, user_email, user_name, auth_method,
                     input_type, dados_detectados, cliente)
                VALUES
                    (%s, %s, %s, %s, 'cadu', 'text', %s, %s)
                RETURNING *
                """,
                (
                    token,
