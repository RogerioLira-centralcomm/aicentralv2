(() => {
  ...
  const enhance = () => {
    ...
    note.innerHTML = `
      <strong>${hasPreviousAnalysis ? 'Dados preservados nesta nova análise' : 'Como a análise funciona'}</strong>
      <span>URL e logo atual continuam protegidos. Edite a URL abaixo ou solte novas referências para complementar a análise.</span>
      ${brand.preserved?.logoUrl ? '<small>Logo principal atual: preservado até você escolher outro na biblioteca da marca.</small>' : ''}
    `;
    form.prepend(note);
