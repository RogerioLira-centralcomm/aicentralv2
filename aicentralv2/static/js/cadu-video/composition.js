...
export function bindComposition(commit,paint){
  ...
  document.addEventListener('cadu:composition-sound',event=>{
    ...
    const row=event.detail;comp().audio.push({sound_id:row.id,name:row.name,start:0,sync_origin:0,in:0,duration:Math.min(row.duration||10,compositionDuration()||10),volume:.35,muted:false,solo:false,fade_in:0,fade_out:0,loop:false,gain_points:[]});changed();
  });
