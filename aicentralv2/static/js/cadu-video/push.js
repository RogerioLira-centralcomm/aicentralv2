import {apiRoot,get,post,studioApi} from './api.js?v=2';
...
export async function enablePush(){
  ...
  try{
    const info=await get(`${studioApi}/push?client_id=${encodeURIComponent(client)}`);
    if(!info.available){notify('O servidor precisa instalar as dependências do worker para enviar notificações.');return;}
    const workerScope=apiRoot.startsWith('/studio/')?'/studio/':'/parametros/';
    const registration=await navigator.serviceWorker.register(`${workerScope}studio-notifications.js`,{scope:workerScope});
    ...
    await post(`${studioApi}/push`,{client_id:client,subscription:subscription.toJSON()});
