(() => {
  ...
  root.querySelector('[data-editor-share]')?.addEventListener('click', async event => { try { const data = await request('/share', {method: 'POST', body: JSON.stringify({enabled: true})}); const base = root.dataset.publicRoot || '/docs/public'; const url = new URL(String(base).replace(/\/$/, '') + '/' + encodeURIComponent(data.document.share_token), window.location.origin).href; await navigator.clipboard?.writeText(url); event.currentTarget.textContent = 'Link público copiado'; show('Link público ativado e copiado.'); } catch (error) { show(error.message); } });
