...
export function showJobs(items=[],exports=[],client='',tasks=[]){
  ...
  const renders=exports.map(row=>`<article class="cadu-processing-job"><strong>Exportação da edição</strong><small>${escape(({ready:'Pronta',queued:'Na fila',rendering:'Renderizando',failed:'Falhou'})[row.status]||row.status)}</small>${row.status==='ready'?`<a href="/parametros/api/format-lab/studio/exports/${escape(row.id)}/content?client_id=${encodeURIComponent(client)}">Baixar MP4</a>`:''}</article>`).join('');
