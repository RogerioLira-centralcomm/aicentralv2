...
| Simulador de investimento | Ativo | `planner.research_plan_inputs`, `planner.simulate_investment` e contexto autorizado | Cenários ilustrativos são calculados em código, somam 100%, respeitam canais mencionados e mantêm valores desconhecidos quando não há verba; catálogo não é cotação. |
...
| Acompanhamento de campanha | Ativo | Anexo/métricas da mensagem; nenhuma ferramenta Reports | Pré-requisito exige anexo ou valor associado a indicador; ano isolado não conta como métrica. Manifesto 1.2.0 removeu a ferramenta Reports não executada. |
