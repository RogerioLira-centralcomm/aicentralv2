...
O catálogo declara os 12 tipos, mas o materializador de respostas textuais cobre nove tipos mais HTML. `project_map` e `link_reader` dependem de fluxos próprios e não devem ser anunciados como disponíveis pela geração genérica. O catálogo de plugins agora expõe sugestões de apresentação, sem conceder ferramentas ou criar artefatos por si. A rota de workflow preserva um `artifact_type` explícito e suas ferramentas de contexto. Ainda é necessário fechar a materialização específica de mapa e referência, a preservação de Markdown fonte e a regra de Quick Scan do Market Intelligence com o agente de artefatos antes de declarar cobertura completa.
