...
- Dezenove casos automatizados cobrem citação literal, fonte apenas descoberta, duplicata lida sem tomar a data do metadado como confirmação, revisão do Quick Scan, métricas, cenários, auditoria aritmética, revisão de página, recibos, expansão do Insights e janela do Radar. A verificação estática de Python e o empacotamento isolado do componente React concluíram sem erro nas etapas anteriores.
