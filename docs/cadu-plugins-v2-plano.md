...
- Insights agora exige trecho literal presente no corpo lido para sustentar a manchete, o insight central, métricas e notícias. IDs de fonte isolados ou excertos de busca não bastam; a verificação literal é uma condição mínima e a adequação semântica continua sob responsabilidade do revisor.
- Quando a primeira busca do Insights não produz fonte recente e legível, uma segunda consulta pública mais ampla tenta cobrir a lacuna antes de retornar indisponibilidade. O Radar também faz uma segunda consulta pública limitada quando não encontra página lida pertinente à marca. O motor compartilhado completo ainda precisa aplicar a mesma disciplina aos demais fluxos.
- O Radar agora usa a janela temporal expressa pelo usuário quando ela corresponde a dia, semana ou mês; na ausência de recorte usa ano. O adaptador de busca ainda não oferece janela arbitrária, então períodos customizados continuam pendentes.
- O Radar agora entrega ao modelo somente páginas lidas que mencionam a marca selecionada ou um concorrente cadastrado. Se a busca não trouxer evidência pertinente, o fluxo informa a lacuna e propõe ampliar o período ou ajustar as entidades; não apresenta resultados genéricos como achados.
...
- Performance agora normaliza métricas coladas no chat e calcula CTR, CPC, CPM e ROAS apenas quando os componentes necessários foram informados uma única vez. O resultado explicita que período e escopo não foram verificados e não transforma cálculos condicionais em dados da plataforma. A ferramenta é somente leitura; o manifesto do Acompanhamento foi atualizado para 1.3.0 na base configurada.
...
- A leitura de plano salvo agora inclui revisão aritmética determinística: soma dos pesos, comparação com a verba do briefing, coerência entre percentual e valor, canais selecionados e duplicatas. Ela retorna achados e escopo explícitos para o modo Auditoria, sem editar o plano. Corrigi também a leitura de verbas com separador de milhar (`R$ 1.000`) no parser compartilhado com o Simulador.
- A auditoria só compara a soma investida com verba fixa explícita. Faixas, tetos e estimativas no briefing permanecem sem valor exato para essa checagem.
- O Simulador respeita os canais explicitamente mencionados. Seu manifesto está na versão 1.2.0; o do Acompanhamento está na 1.3.0 na base configurada, alinhando as ferramentas declaradas aos caminhos efetivos.
- O Revisor de página agora aceita URL `http` ou `https` e conteúdo colado. Sua instrução separa elemento observado, hipótese, correção e forma de verificar; leitura de texto não é tratada como teste visual ou de acessibilidade.
- Conceito criativo agora pede duas rotas comparáveis com prova, risco e critério de escolha; Copy por canal separa mensagem confirmada de proposta e só aplica limites de caracteres fornecidos ou configurados. Essas são melhorias editoriais no prompt, não um revisor independente nem validação automática de caracteres.
- Operações já tinha confirmação, idempotência e recibos para tarefas. A execução agora exige IDs e quantidade coerente no recibo antes de anunciar a criação do lote como concluída; falta ainda revisar os contratos das demais ações e os estados de recuperação.
- Atendimento e entregas agora recebe um resumo determinístico dos estados das tarefas nativas, com contagens e lacunas. O contrato indica que tarefa concluída não comprova entrega ou aceite do cliente, e não infere mudanças desde um status anterior sem histórico.
- Vinte e dois casos automatizados cobrem citação literal, fonte apenas descoberta, duplicata lida sem tomar a data do metadado como confirmação, revisão do Quick Scan, métricas, cenários, auditoria aritmética, revisão de página, recibos, status de tarefas, expansão do Insights e seleção de fontes do Radar. A verificação estática de Python e o empacotamento isolado do componente React concluíram sem erro nas etapas anteriores.
- Permanecem para execução: motor compartilhado completo de pesquisa, expansão por lacunas entre modos, revisão estratégica e editorial além das checagens numéricas, revisão de campanhas com períodos confirmados, contratos das demais ações e migração completa das rotas. A revisão do Quick cobre apenas Market Intelligence. Esta lista descreve trabalho aberto, não promessa de que os cinco fluxos já estão prontos.
