...
- Onze casos automatizados cobrem citação literal, fonte apenas descoberta, duplicata lida sem tomar a data do metadado como confirmação, revisão do Quick Scan, métricas, cenários, revisão de página, recibos e expansão do Insights. A verificação estática de Python e o empacotamento isolado do componente React concluíram sem erro nas etapas anteriores.
