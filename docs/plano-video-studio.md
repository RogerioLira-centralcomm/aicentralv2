...
| Geração | Aba Seedance com modelo do servidor, proporção, duração, qualidade/resolução, movimento, intensidade, áudio composto e seed | Verificação real no provedor com custo e observação do áudio produzido |
...
## Continuação — agente e imagem para vídeo

O Studio agora inclui uma aba **Agente**. O pedido em linguagem natural passa por um planejador no backend, é reduzido a uma lista fechada de alterações suportadas e volta para revisão. O usuário precisa usar **Aplicar ao projeto** e a geração paga continua separada no botão **Gerar com IA**. O plano não pode excluir mídia, exportar ou executar comandos.

A skill `seedance-2-5-image-to-video` foi incorporada ao contrato do agente e às capacidades do Studio. O modo **Uma imagem** usa uma única peça selecionada, duração de 4 a 30 segundos nas opções admitidas, saída 720p, áudio nativo opcional e sem seed. O formato da peça é adotado como padrão ao selecioná-la. O usuário pode escolher outra saída; nesse caso, o pipeline enquadra a imagem no formato escolhido antes de enviá-la, e o vídeo herda essa proporção preparada.

Com uma única peça, o Studio entra automaticamente em **Uma imagem** e permite gerar o vídeo. Como duas tomadas costumam produzir uma narrativa publicitária melhor, o painel da cena oferece **Criar cena 2 com Trocr**. A ação explícita gera uma imagem nova usando créditos, preserva elenco, produto, marca, texto, cores e elementos gráficos, varia enquadramento e ambiente, adiciona o resultado à biblioteca e troca o projeto para storyboard.

O painel de geração agora separa o áudio em camadas combináveis: ambiente/efeitos, narração e música de fundo. A narração pode ser orientada ao áudio nativo do Seedance ou escrita como texto exato para TTS e mixagem posterior. O Studio sugere texto e direção de voz a partir do conteúdo das cenas, respeita o orçamento de palavras da duração e mantém a sugestão editável. Música e ambiente podem acompanhar qualquer uma das duas formas de narração; na locução exata, o prompt bloqueia fala nativa e a mixagem aplica ducking à trilha.

O agente lateral identifica o clipe aberto e pode planejar corte, velocidade de 0,25× a 4×, volume, fades, preto e branco e espelhamento. A aplicação altera a prévia e exige exportação explícita. O workspace mantém um ledger idempotente com gerações de vídeo, locuções incluídas na cotação, cenas Trocr e exportações locais. Valores em processamento aparecem separados; somente operações concluídas entram no gasto confirmado. Exportações FFmpeg ficam registradas com custo de IA igual a zero.

Validação: 60 testes passaram, módulos JavaScript passaram na verificação de sintaxe e o CSS foi recompilado. No preview de navegador, foram conferidos: carregamento do agente, cabeçalho ao trocar de aba, atalhos de pedido, seleção de imagem única e alteração da saída de 16:9 para 9:16 com resumo `Uma imagem · 8s · 9:16 · 720p · Com áudio`. O backend também confirma que 9:16 produz plano de 720×1280 com áudio nativo e carrega a skill pelo catálogo interno. O teste não disparou uma geração paga.
