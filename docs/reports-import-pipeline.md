...
- A interface **Importações** mostra lote e até 100 linhas de prévia, com pendências primeiro. O operador pode corrigir os campos de uma linha, informar uma justificativa e confirmá-la. `add_reports_import_decisions_v1.sql` guarda a versão anterior, a versão confirmada, usuário e data da decisão. Prints entram em `awaiting_extraction`; a ação explícita **Ler print com IA** produz blocos com identidade, período, métricas brutas e evidência visível, salvos em `add_reports_import_visual_runs_v1.sql`. A leitura é cobrada em créditos Cadu e gera sugestões sem consolidar números.
- `add_reports_import_projection_v1.sql` cria uma projeção conservadora por campanha, dia e métrica. Reenvios com o mesmo valor são reduzidos a um fato; valores distintos ficam sem valor projetado e aumentam o contador de conflitos. `/connect/api/v1/reports/import-metrics` mostra totais por período e plataforma no painel, separados do Google Ads Script. Custos só aparecem como total quando há uma única moeda no recorte. A projeção é calculada sobre as observações imutáveis, portanto uma revisão posterior não apaga a origem.
- `add_reports_import_projection_decisions_v1.sql` guarda a escolha do operador entre versões divergentes, com justificativa e o último ID de observação visto. A tela mostra cada valor com arquivo de origem. Uma nova observação após a decisão reabre o conflito; a escolha anterior continua auditável. O endpoint sempre valida `client_id`, campanha, data, métrica e observação antes de gravar.
- Prints normalizados podem ser vistos pelo operador dentro do cliente. Blocos que o leitor visual identificou como **um dia exato** têm um formulário de confirmação de IDs, data, moeda e métricas; a decisão humana gera uma linha vinculada à imagem e observações diárias na mesma projeção dos CSV/XLSX. Blocos de intervalo e período indefinido continuam como evidência, sem conversão automática em dado diário.
- Blocos de **intervalo** podem ser confirmados com início, fim, identidade, métricas e justificativa. `add_reports_import_range_snapshots_v1.sql` guarda o snapshot e as métricas com vínculo ao bloco do print; a tela lista até 100 snapshots recentes por cliente. Esses valores não entram na série diária, não são somados entre intervalos sobrepostos e não são transformados em incrementos sem uma regra de comparabilidade.
- Arquivos com cabeçalhos desconhecidos exibem **Mapear colunas**. O operador relaciona colunas reais aos campos canônicos uma vez para todas as linhas pendentes, com plataforma/moeda/formato de data opcionais. `add_reports_import_column_maps_v1.sql` guarda o mapa, usuário, justificativa e quantidade de linhas aplicadas. As linhas brutas não são reescritas; apenas a interpretação e suas observações são atualizadas. Linhas já confirmadas são preservadas.
- **Sugerir colunas com TypeSafe** usa perguntas Choice independentes em uma chamada System One para até 16 cabeçalhos desconhecidos. O retorno tipado, probabilidades, modelo e uso são guardados em `add_reports_import_column_suggestions_v1.sql`. As sugestões apenas preenchem seletores quando o operador escolhe usá-las; aplicar o mapa exige confirmação separada. Sem credencial TypeSafe configurada, o mapeamento manual continua disponível.

Próximos blocos: comparação explícita entre snapshots de intervalo e evidência por região do print. Um arquivo real de cada plataforma é necessário para ampliar os aliases sem adivinhar o formato do export.
