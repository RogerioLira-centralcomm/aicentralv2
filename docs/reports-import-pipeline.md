...
- A interface **Importações** mostra lote e até 100 linhas de prévia, com pendências primeiro. O operador pode corrigir os campos de uma linha, informar uma justificativa e confirmá-la. `add_reports_import_decisions_v1.sql` guarda a versão anterior, a versão confirmada, usuário e data da decisão. Prints entram em `awaiting_extraction`; a ação explícita **Ler print com IA** produz blocos com identidade, período, métricas brutas e evidência visível, salvos em `add_reports_import_visual_runs_v1.sql`. A leitura é cobrada em créditos Cadu e gera sugestões sem consolidar números. Observações tabulares ficam prontas para reconciliação; os totais do painel continuam independentes delas até existir uma regra de projeção para snapshots sobrepostos.

Próximos blocos: revisão/confirmação dos blocos visuais com evidência por região, mapeamento assistido de cabeçalhos desconhecidos e projeção versionada por campanha, dia e métrica. Um arquivo real de cada plataforma é necessário para ampliar os aliases sem adivinhar o formato do export.
