...
- A interface **Importações** mostra lote e até 100 linhas de prévia, com pendências primeiro. O operador pode corrigir os campos de uma linha, informar uma justificativa e confirmá-la. `add_reports_import_decisions_v1.sql` guarda a versão anterior, a versão confirmada, usuário e data da decisão. Prints ficam em `awaiting_extraction` e não alimentam métricas ainda. Observações tabulares ficam prontas para reconciliação; os totais do painel continuam independentes delas até existir uma regra de projeção para snapshots sobrepostos.
