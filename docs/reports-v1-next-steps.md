...
| P2.1–P2.2 | Primeira fatia codificada: caixa de entrada por cliente, CSV/XLSX, prints preservados, leitura visual sob demanda com evidência, identificação por IDs exatos em tabelas, observações imutáveis e confirmação manual auditável de linhas ambíguas. Confirmação de blocos visuais, projeção incremental consolidada e arquivos reais de plataformas ainda pendentes. Nenhuma API de mídia adicional é usada. |
