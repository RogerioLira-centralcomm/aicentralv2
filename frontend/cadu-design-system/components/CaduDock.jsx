...
function DockPicker({candidates, items, busy, query, onQueryChange, linkUrl, onLinkUrlChange, linkTitle, onLinkTitleChange, onSave, onRemove, onClose, identity}) {
  ...
  return <div className="cadu-ds-dock-picker-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="cadu-ds-dock-picker" role="dialog" aria-modal="true" aria-labelledby="cadu-dock-picker-title">
      <header className="cadu-ds-dock-picker__header"><div><h2 id="cadu-dock-picker-title">Adicionar à dock</h2><p>Seus atalhos, sempre à mão.</p></div><button type="button" className="cadu-ds-dock-picker__close" onClick={onClose} aria-label="Fechar"><Icon name="close" size={18}/></button></header>
      <div className="cadu-ds-dock-picker__tabs" role="tablist" aria-label="Gerenciar dock"><button type="button" role="tab" aria-selected={tab === 'add'} onClick={() => setTab('add')}>Adicionar</button><button type="button" role="tab" aria-selected={tab === 'saved'} onClick={() => setTab('saved')}>Fixados <span>{saved.length}</span></button></div>
      <div className="cadu-ds-dock-picker__content">{tab === 'add' ? <>
        <label className="cadu-ds-dock-picker__search"><span>Buscar marca, projeto ou recurso</span><input autoFocus value={query} onChange={event => onQueryChange(event.target.value)} placeholder="Buscar item"/></label>
