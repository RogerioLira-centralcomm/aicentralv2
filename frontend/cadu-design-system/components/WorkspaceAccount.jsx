...
function Plan({account, urls}) {
  ...
  return <div className="cadu-ds-account-stack"><AccountPageHeader eyebrow="Plano do cliente" title="Escolha o ritmo da operação" description="Compare capacidade, equipe e suporte. O plano atual aparece destacado."/><section className="cadu-ds-plan-current"><div><span>Plano atual</span><strong>{currentPlan}</strong><p>{number(insight.users?.used)} pessoas ativas · {number(insight.tokens?.limit)} tokens mensais</p></div><a href={urls.usage}>Ver uso</a></section><section className="cadu-ds-plan-grid">{cards.map((option, index) => { const name = option.plan_name || option.plan_definition_name || option.plan_type || `Plano ${index + 1}`; const isCurrent = option.is_current || name === currentPlan; return <article className={isCurrent ? 'is-current' : ''} key={option.id || name}><span>{isCurrent ? 'Plano atual' : index === 0 ? 'Comece por aqui' : 'Para crescer'}</span><h3>{name}</h3><p>{number(option.tokens_monthly_limit || option.pd_tokens_monthly_limit || 0)} tokens mensais e até {number(option.max_users || option.pd_max_users || 0)} pessoas.</p><ul><li>Contexto compartilhado da equipe</li><li>Projetos e marcas organizados</li><li>Uso acompanhado por período</li></ul><a href={isCurrent ? urls.usage : urls.team}>{isCurrent ? 'Acompanhar plano' : 'Falar com o time'}</a></article>; })}</section></div>;
}

function Usage({account}) {
  const insight = account.insights || {};
  return <div className="cadu-ds-account-stack"><AccountPageHeader eyebrow="Uso do cliente" title="Consumo da operação" description="Acompanhe o ciclo atual e o consumo confirmado pelas ferramentas Cadu."/><section className="cadu-ds-account-metrics"><Metric label="Tokens neste ciclo" value={`${number(insight.tokens?.used)} de ${number(insight.tokens?.limit)}`}><progress value={insight.tokens?.percentage || 0} max="100"/></Metric><Metric label="Execuções recentes" value={number(account.movements?.length)} detail="registros confirmados"/><Metric label="Consumo confirmado" value={number((account.movements || []).reduce((total, item) => total + Number(item.amount || 0), 0))} detail="tokens debitados"/></section><section className="cadu-ds-account-section"><header><div><span>Consumo confirmado</span><h2>Atividade recente</h2></div></header><DataTable columns={['Execução', 'Data', 'Referência', 'Créditos']} empty="Quando houver uso confirmado, ele aparecerá aqui." rows={(account.movements || []).map((item, index) => <tr key={item.id || index}><td><b>{item.reason || 'Execução Cadu'}</b></td><td>{dateTime(item.created_at)}</td><td>{item.reference || '—'}</td><td>−{number(item.amount)}</td></tr>)}/></section></div>;
}

function Credits({account}) {
  const available = account.credit?.available ?? account.position?.available ?? 0;
  return <div className="cadu-ds-account-stack"><AccountPageHeader eyebrow="Créditos do cliente" title="Saldo disponível" description="Acompanhe os lotes compartilhados e o consumo mais recente da operação."/><CreditSummary available={available} lots={account.purchases?.length} lastMovement={account.movements?.[0]}/><section className="cadu-ds-account-section"><header><div><span>Saldo por lote</span><h2>Lotes ativos</h2></div></header><DataTable columns={['Pacote', 'Vencimento', 'Disponível', 'Total']} empty="Não há lotes ativos no momento." rows={(account.purchases || []).map((lot, index) => <tr key={lot.id || index}><td><b>{lot.package_name}</b></td><td>{lot.expires_at ? civilDate(lot.expires_at) : 'Sem vencimento'}</td><td>{number(lot.available)}</td><td>{number(lot.credits)}</td></tr>)}/></section></div>;
}
