...
function ListBlock({title, items}) {
  ...
  return <section className="cadu-ds-brand-data-block"><h3>{title}</h3><ul>{items.map((item, index) => <li key={`${title}-${index}`}>{typeof item === 'string' ? item : item.name || item.value || item.address || item.title}</li>)}</ul></section>;
}
