...
function AuditHistory({history}) {
  ...
  return <section className="cadu-ds-brand-audit-history" id="auditoria" aria-label="Auditorias realizadas"><header><div><p>Rastreabilidade</p><h2>Auditorias realizadas</h2><span>Histórico operacional de fontes, coleta, custo estimado e revisão.</span></div></header><div>{history.map(item => { const collected = item.collected_data || {}; const costs = item.costs || {}; const effort = item.human_effort || {}; const reliability = item.reliability || {}; const measured = Number(reliability.provider_calls || 0) > 0; const successRate = measured ? Math.round(Number(reliability.successful_calls || 0) * 100 / Number(reliability.provider_calls)) : null; return <details key={item.job_id || item.created_at}><summary><span>{label(item.analysis_mode)}</span><b>{date(item.created_at)}</b><small>{item.status === 'failed' ? 'não concluída' : item.status === 'queued' ? 'na fila' : 'revisada'}</small></summary><dl><div><dt>Dados coletados</dt><dd>{collected.fields?.length ? collected.fields.join(' · ') : item.status === 'failed' ? 'Não concluídos' : 'Em processamento'}</dd></div><div><dt>Coerência</dt><dd>{collected.coherence === 'reviewed' ? 'revisada por agentes' : collected.coherence === 'needs_review' ? 'requer revisão humana' : item.status === 'failed' ? 'não avaliada' : 'em processamento'}</dd></div><div><dt>Custo Cadu</dt><dd>{costs.provider_tokens || costs.estimated_tokens ? `${Number(costs.provider_tokens || costs.estimated_tokens).toLocaleString('pt-BR')} tokens` : 'estimativa pendente'}</dd></div><div><dt>Esforço equivalente</dt><dd>{effort.estimated_person_hours ? `${effort.estimated_person_hours} h de pesquisa e consolidação` : 'a calcular'}</dd></div>{measured && <div><dt>Confiabilidade dos modelos</dt><dd>{successRate}% das chamadas · {reliability.fallback_used ? 'fallback acionado' : 'rota principal'}{reliability.partial_result ? ' · resultado parcial' : ''}</dd></div>}</dl>{item.sources?.length ? <div className="cadu-ds-brand-audit-history__sources"><b>Fontes e links pesquisados</b><ul>{item.sources.slice(0, 12).map((source, index) => <li key={`${source.url}-${index}`}><a href={source.url} target="_blank" rel="noreferrer">{source.title || source.url}</a></li>)}</ul></div> : null}{item.reviews?.length ? <small>{item.reviews.length} agentes atuaram em pesquisa, extração e revisão.</small> : null}</details>; })}</div></section>;
}

function ReliabilitySummary({summary}) {
  if (!summary?.measured_runs) return null;
  return <section className="cadu-ds-brand-reliability" aria-label="Confiabilidade das análises">
    <header><div><p>Saúde da automação</p><h2>Confiabilidade medida nesta marca</h2><span>Indicadores calculados somente sobre execuções que registraram telemetria de provedor.</span></div></header>
    <dl><div><dt>Chamadas concluídas</dt><dd>{summary.call_success_rate}%</dd><small>{summary.successful_calls} de {summary.provider_calls}</small></div><div><dt>Execuções estáveis</dt><dd>{summary.stable_run_rate}%</dd><small>{summary.measured_runs - summary.partial_runs} de {summary.measured_runs}</small></div><div><dt>Fallback</dt><dd>{summary.fallback_runs}</dd><small>execuções recuperadas</small></div><div><dt>Resultados parciais</dt><dd>{summary.partial_runs}</dd><small>exigem atenção</small></div></dl>
  </section>;
}
...
export function WorkspaceBrand({bootstrap}) {
  ...
  const reliabilitySummary = brand.reliabilitySummary || {};
