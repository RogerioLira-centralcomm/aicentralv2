  ...
  return <BrandDialog title="Atualizar análise da marca" detail="A identidade atual permanece protegida até você revisar e aprovar uma nova versão." onClose={onClose} className="cadu-ds-brand-audit-dialog">
    <form className="cadu-ds-brand-form" method="post" encType="multipart/form-data" action={urls.audit}>
      ...
      <div className="cadu-ds-brand-audit-dialog__columns"><div>
        ...
        {brand.assets?.length > 0 && <div className="cadu-ds-brand-audit-existing"><Hidden name="existing_assets_present" value="true"/><strong>Escolha os ativos desta análise</strong><span>Use o material já aprovado e envie arquivos somente quando houver algo novo.</span><div>{brand.assets.slice(0, 12).map(asset => <label key={asset.id}><input type="checkbox" name="existing_asset_ids" value={asset.id} defaultChecked={asset.isPrimary || asset.status === 'approved'}/>{asset.displayUrl && <img src={asset.displayUrl} alt=""/>}<small>{asset.metadata?.label || assetLabels[asset.role] || 'Ativo'}</small></label>)}</div></div>}
...
export function WorkspaceBrand({bootstrap}) {
  ...
  const brandNav = [
    ...
    ...(!isProcessing && readinessScore < 50 ? [{id:'completar', label:'Completar análise', icon:'pulse'}] : []),
    ...
    <main className="cadu-ds-home-main">
      <div className="cadu-ds-home-workarea cadu-ds-brand-workarea">
        ...
        <section className="cadu-ds-brand-content">
          ...
          {!isProcessing && <BrandCompletion score={readinessScore} missing={brand.readiness?.missing || []} processing={false} onAudit={() => setDialog('audit')} onEdit={() => setDialog('identity')}/>}
