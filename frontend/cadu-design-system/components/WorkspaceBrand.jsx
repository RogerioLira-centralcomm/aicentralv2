...
function FilledReading({items, emptyLabel = 'Adicione contexto para orientar as próximas decisões.'}) {
  ...
  return filled.length ? <div className="cadu-ds-brand-reading cadu-ds-brand-reading--direction">{filled.map(item => { const value = String(item.value); const parts = value.length > 240 ? value.match(/[^.!?]+[.!?]+|[^.!?]+$/g)?.map(part => part.trim()).filter(Boolean) || [value] : [value]; return <article key={item.label} className={parts.length > 1 ? 'is-long' : ''}><small>{item.label}</small>{parts.length > 1 ? <div className="cadu-ds-brand-readable-copy">{parts.map((part, index) => <p key={`${item.label}-${index}`}>{part}</p>)}</div> : <b>{value}</b>}</article>; })}</div> : <div className="cadu-ds-brand-reading-empty">{emptyLabel}</div>;
}
  ...
  return <div className={`cadu-ds-home-shell cadu-ds-brand-shell is-${lifecycle}`}>
    <main className="cadu-ds-home-main">
      <div className="cadu-ds-home-workarea cadu-ds-brand-workarea">
        ...
        <EntityNavigator label={brand.name || 'Marca'} items={brandNav} identity={<><VisualIdentity src={brand.logoUrl} initials={brand.initials || brand.name} label={brand.name} color={brand.primaryColor} imageTreatment="brand"/><span><small>Marca</small><b>{brand.name}</b></span></>}>
          {!isProcessing && <>
            <span>Gestão</span>
