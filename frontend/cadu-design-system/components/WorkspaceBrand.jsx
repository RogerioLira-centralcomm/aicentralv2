  ...
  return <BrandDialog title="Atualizar análise da marca" detail="A identidade atual permanece protegida até você revisar e aprovar uma nova versão." onClose={onClose} className="cadu-ds-brand-audit-dialog">
    <form className="cadu-ds-brand-form" method="post" encType="multipart/form-data" action={urls.audit}>
      ...
      <div className="cadu-ds-brand-audit-dialog__columns"><div>
        ...
        {reusableAssets.length > 0 && <div className="cadu-ds-brand-audit-existing"><Hidden name="existing_assets_present" value="true"/><strong>Escolha os ativos desta análise</strong><span>Use o material já aprovado e envie arquivos somente quando houver algo novo.</span><div>{reusableAssets.slice(0, 12).map(asset => <label key={asset.id}><input type="checkbox" name="existing_asset_ids" value={asset.id} defaultChecked/><img src={asset.displayUrl} alt=""/><small>{asset.metadata?.label || assetLabels[asset.role] || 'Ativo'}</small></label>)}</div></div>}
