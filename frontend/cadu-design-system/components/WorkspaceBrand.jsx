...
function IdentityDialog({brand, urls, csrfToken, onClose}) {
  return <BrandDialog title="Editar dados básicos" detail="A direção estratégica pode ser atualizada em conversa com o Cadu, preservando histórico e fontes." onClose={onClose}>
    <form className="cadu-ds-brand-form" method="post" action={urls.updateIdentity}>
      ...
      <small className="cadu-ds-brand-form__note">Logo e referências são administradas na biblioteca visual desta página.</small>
      <footer><button type="button" onClick={onClose}>Cancelar</button><button className="is-primary">Salvar dados</button></footer>
    </form>
  </BrandDialog>;
}
...
export function WorkspaceBrand({bootstrap}) {
  ...
  const brandNav = [
    ...
    ...(readinessScore < 50 ? [{id:'completar', label:'Completar análise', icon:'pulse'}] : []),
    {id:'direcao', label:'Direção da marca', icon:'compose'},
    {id:'inteligencia', target:'direcao', label:'Inteligência', icon:'pulse'},
    {id:'biblioteca', label:'Biblioteca visual', icon:'file', count:(brand.assets || []).length},
    {id:'campanhas', target:'biblioteca', label:'Campanhas e projetos', icon:'folder', count:campaigns.length + linkedProjects.length},
    {id:'auditoria', target:'biblioteca', label:'Auditoria', icon:'history', count:auditHistory.length},
  ];
  const brandRailGroups = [
    ...
    {title:'Arquivos principais', items:(brand.assets || []).map(item => ({...item, title:item.metadata?.label || assetLabels[item.role] || 'Arquivo de marca', detail:item.mimeType || item.sourceKind}))},
  ...
  return <div className={`cadu-ds-home-shell cadu-ds-brand-shell is-${lifecycle}`}>
    <main className="cadu-ds-home-main">
      <div className="cadu-ds-home-workarea cadu-ds-brand-workarea">
        ...
        <EntityNavigator label={brand.name || 'Marca'} items={brandNav}/>
        <section className="cadu-ds-brand-content">
          <a className="cadu-ds-brand-back" href={bootstrap.urls.brands}>← Marcas</a>
          <header className="cadu-ds-brand-hero" id="marca-visao"><div className="cadu-ds-brand-hero__identity"><VisualIdentity src={brand.logoUrl} initials={brand.initials || brand.name} label={brand.name} color={brand.primaryColor} imageTreatment="brand"/></div><div className="cadu-ds-brand-hero__copy"><p>{brand.sector || 'Identidade de marca'}</p><h1>{brand.name}</h1>{(profile.brandSummary || profile.positioning) && <span>{profile.brandSummary || profile.positioning}</span>}<div className="cadu-ds-brand-hero__meta"><span className={`cadu-ds-brand-status is-${lifecycle}`}>{lifecycle === 'approved' ? 'Aprovada' : lifecycle === 'pending_approval' ? 'Revisão pendente' : lifecycle === 'audit_processing' ? 'Em análise' : lifecycle === 'audit_failed' ? 'Análise não concluída' : lifecycle === 'data_available_unverified' ? 'Dados não verificados' : 'Sem auditoria'}</span>{brand.websiteUrl && <a href={brand.websiteUrl} target="_blank" rel="noreferrer">Site oficial</a>}</div></div><div className="cadu-ds-brand-hero__actions">{verified ? <button type="button" className="is-primary" onClick={openConversation}>Conversar sobre a marca</button> : canEdit && !isProcessing ? <button type="button" className="is-primary" onClick={() => setDialog('audit')}>{lifecycle === 'audit_failed' ? 'Corrigir e tentar novamente' : 'Auditar informações'}</button> : null}{canEdit && <button type="button" onClick={() => setDialog('identity')}>Editar dados</button>}<details><summary>Mais ações</summary><div><button type="button" onClick={() => setDialog('link')}>Vincular projeto</button>{showDossier && <><a href={urls.createImage}>Criar imagem</a><a href={urls.createVideo}>Criar vídeo</a><a href={urls.createPlan}>Criar plano</a></>}<a href={urls.system}>Sistema avançado</a>{canEdit && <button type="button" className="is-danger" onClick={() => setDialog('delete')}>Apagar marca</button>}</div></details></div></header>
          <BrandCompletion score={readinessScore} missing={brand.readiness?.missing || []} processing={isProcessing} onAudit={() => setDialog('audit')} onEdit={() => setDialog('identity')}/>
          {showDossier && lifecycle !== 'data_available_unverified' && <section className={`cadu-ds-brand-review cadu-ds-brand-review--${status || 'idle'}`}><div><p>Próximo passo</p><h2>{reviewTitle}</h2><span>{reviewDescription}</span></div><div>{canShowSynthesis && <button type="button" onClick={() => setDialog('reviews')}>Ver síntese da análise</button>}{status === 'pending_approval' && canEdit && <form method="post" action={urls.approve}><Hidden name="_csrf" value={bootstrap.csrf}/><button className="is-primary">Aprovar síntese</button></form>}</div></section>}
          ...
          />}
          {showDossier ? <div className="cadu-ds-brand-layout">
