...
function ProjectGettingStarted({project, conversationUrl, canEdit, onEditContext}) {
  ...
  const actions = [
    {key:'understand', title:'Entender o projeto', detail:'Veja o que está registrado, o que falta e de onde vem cada informação.', icon:'analysis'},
    {key:'verify', title:'Conferir dados e fontes', detail:'Encontre divergências, fontes não indexadas e lacunas no contexto.', icon:'source'},
  ...
  return <section className="cadu-ds-project-getting-started" aria-labelledby="project-getting-started-title">
    <header><div><span>Para começar</span><h2 id="project-getting-started-title">Conheça e complete este projeto</h2><p>O Cadu consulta os dados do projeto, as fontes disponíveis e as conversas relacionadas. As três primeiras ações respondem no chat; o guia vira um rascunho editável somente quando você escolhe essa opção.</p></div></header>
    ...
    <footer><span><ProjectIcon name="context"/><b>{customCount}</b> campo{customCount === 1 ? '' : 's'} personalizado{customCount === 1 ? '' : 's'}</span><span><ProjectIcon name="source"/>{files.length ? <><b>{ready} de {files.length}</b> arquivos indexados{working > 0 ? ` · ${working} processando` : ''}{attention > 0 ? ` · ${attention} para revisar` : ''}</> : 'Nenhum arquivo indexado'}</span>{canEdit ? <button type="button" onClick={onEditContext}>Revisar contexto salvo</button> : <a href="#direcao">Ver contexto salvo</a>}</footer>
  </section>;
}
