...
export function ChatContextSelector({context = {}, projects = [], brands = [], onProjectChange, onBrandChange, disabled = false, loading = false, projectsOnly = false}) {
  ...
  return <details ref={root} open={open} onToggle={event => setOpen(event.currentTarget.open)} className="cv-context-selector">
    ...
    <div className="cv-context-selector__menu" aria-label="Contextos disponíveis">
      <button type="button" className={!context.project_ref && !context.brand_ref ? 'is-active' : ''} onClick={() => chooseProject('')} disabled={disabled}><span className="cv-context-selector__personal">↗</span><span><b>Sessão livre</b></span>{!context.project_ref && !context.brand_ref && <em>✓</em>}</button>
