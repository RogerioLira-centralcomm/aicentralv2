...
function Thread({messages, onPrompt, onOpenArtifact, onOpenResource, onDecision, onOpenDiagnostics, running, runtime}) {
  ...
  return <div className="cv-mx-auto cv-w-full cv-max-w-[820px] cv-px-6 cv-pb-40 cv-pt-10 md:cv-px-10">
...
function Composer({value, onChange, onSubmit, onAttach, attachments, onRemoveAttachment, running, onStop, contextLabel}) {
  ...
  return <div className="cv-pointer-events-none cv-absolute cv-inset-x-0 cv-bottom-0 cv-z-20 cv-bg-gradient-to-t cv-from-ink cv-from-65% cv-to-transparent cv-px-4 cv-pb-5 cv-pt-14 md:cv-px-8">
    <form onSubmit={event => { event.preventDefault(); onSubmit(); }} className="cv-pointer-events-auto cv-mx-auto cv-w-full cv-max-w-[760px] cv-rounded-2xl cv-border cv-border-white/10 cv-bg-[#0f1d20]/95 cv-p-2 cv-shadow-composer cv-backdrop-blur-xl">
      ...
      <textarea ref={textarea} value={value} onChange={event => onChange(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} rows="1" maxLength="20000" placeholder="Pergunte ou peça uma alteração…" aria-label="Mensagem para o Cadu" className="cv-block cv-max-h-[150px] cv-min-h-[48px] cv-w-full cv-resize-none cv-border-0 cv-bg-transparent cv-px-3 cv-py-3 cv-text-[15px] cv-leading-6 cv-text-white cv-outline-none placeholder:cv-text-[#6f8985]"/>
      <div className="cv-flex cv-items-center cv-justify-between cv-px-1 cv-pb-1">
        <div className="cv-flex cv-min-w-0 cv-items-center cv-gap-2"><button type="button" onClick={onAttach} className="cv-grid cv-h-9 cv-w-9 cv-place-items-center cv-rounded-xl cv-border-0 cv-bg-transparent cv-text-mist hover:cv-bg-white/[.06]" aria-label="Anexar arquivo"><Icon name="attach" size={17}/></button><span className="cv-overflow-hidden cv-text-ellipsis cv-whitespace-nowrap cv-text-[11px] cv-text-[#77918d]">{contextLabel}</span></div>
...
export function Conversation({title, context, projects, onProjectChange, contextLoading, runtime, diagnostics, messages, input, setInput, onSubmit, onAttach, attachments, onRemoveAttachment, running, onStop, onNew, onPrompt, onOpenArtifact, onOpenResource, onDecision, mobileMenu, artifactOpen, notice, onDismissNotice}) {
  ...
  return <section className="cv-relative cv-flex cv-min-w-0 cv-flex-1 cv-flex-col cv-bg-ink">
    <header className="cv-flex cv-h-[68px] cv-flex-none cv-items-center cv-gap-4 cv-border-b cv-border-white/[.07] cv-px-4 md:cv-px-6">
      ...
      <h1 className={`cv-m-0 cv-min-w-0 cv-flex-1 cv-overflow-hidden cv-text-ellipsis cv-whitespace-nowrap cv-text-[15px] cv-font-semibold cv-tracking-[-.01em] ${artifactOpen ? 'cv-hidden 2xl:cv-block' : ''}`} title={title}>{title}</h1>
    ...
    <div className="cv-scroll cv-min-h-0 cv-flex-1 cv-overflow-y-auto"><Thread messages={messages} onPrompt={onPrompt} onOpenArtifact={onOpenArtifact} onOpenResource={onOpenResource} onDecision={onDecision} running={running} runtime={runtime} onOpenDiagnostics={() => { if (details.current) details.current.open = true; }}/></div>
