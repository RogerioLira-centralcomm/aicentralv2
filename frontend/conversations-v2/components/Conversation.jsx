...
function Answer({message, onPrompt, onOpenArtifact, onOpenResource, onRevisitPrompt, creditsUrl, caduMark, interactive}) {
  ...
  return <div className="cv-message-enter cv-assistant-answer cv-max-w-[72ch]">
    {response.plugin?.name && <div className="cv-plugin-used" aria-label={`Plugin utilizado: ${response.plugin.name}`}><span className="cv-plugin-used__mark">{caduMark ? <img src={caduMark} alt=""/> : <Icon name="brand" size={12}/>}</span><span>Plugin utilizado: <strong>{response.plugin.name}</strong></span></div>}
...
function Thread({messages, interaction, onPrompt, onOpenArtifact, onOpenResource, onDecision, onRevisitPrompt, creditsUrl, onOpenDiagnostics, running, runtime, diagnostics, activePlugin, caduMark, starterProject, starterBrand, starterHome}) {
...
export function Conversation({inactive, layout, viewport, shellV2 = true, conversationId, title, context, projects, brands, caduMark, starterProject, starterBrand, starterHome, contextLoading, runtime, diagnostics, activePlugin, messages, input, setInput, onSubmit, attachments, onRemoveAttachment, executionMode, onExecutionModeChange, running, onStop, onPrompt, onOpenArtifact, onOpenResource, onDecision, onRevisitPrompt, creditsUrl, onOpenHistory, historyOpen, artifactOpen, composerContext, onClearContext, onAttach, onContextDrop, onProjectChange, onCreateProject, queuedTurns, onUpdateQueuedTurn, onRemoveQueuedTurn, onMoveQueuedTurn, onOpenLibrary, automation, audioTranscriptionEndpoint, csrfToken}) {
  ...
  return <section inert={inactive ? '' : undefined} aria-hidden={inactive ? 'true' : undefined} className={`cv-conversation-shell cv-relative cv-flex cv-min-w-0 cv-flex-1 cv-flex-col cv-bg-ink ${!messages.length && !running ? 'cv-conversation--empty' : ''}`}>
    <header className="cv-conversation-header cv-relative cv-z-50 cv-flex cv-h-[68px] cv-flex-none cv-items-center cv-gap-4 cv-px-4 md:cv-px-6">
      ...
      <span className="cv-conversation-context"><span className="cv-hidden cv-text-[11px] cv-text-[#78908c] sm:cv-inline">Contexto</span><ChatContextSelector context={context} projects={projects} brands={brands} onProjectChange={onProjectChange} disabled={running} loading={contextLoading} projectsOnly/><button type="button" className="cv-create-project-button" onClick={onCreateProject} disabled={running || contextLoading} aria-label="Criar projeto" title="Criar projeto"><Icon name="plus" size={15}/></button></span>
