...
export function ExecutionQueue({items = [], onUpdate, onRemove, onMove}) {
  ...
  return <section className={`cv-execution-queue ${expanded ? 'is-expanded' : ''}`} aria-label={`${items.length} pedido${items.length === 1 ? '' : 's'} na fila`}>
    <header><span><b>Fila de execução</b><small>{items.length}/5 · próximo: {items[0]?.prompt}</small></span><button type="button" className="cv-execution-queue__toggle" aria-expanded={expanded} onClick={() => setExpanded(value => !value)}>{expanded ? 'Recolher' : 'Ver fila'}<Icon name="chevron" size={12}/></button></header>
    <ol>
      {items.map((item, index) => <li key={item.id}>
        ...
        <div className="cv-execution-queue__actions">
          <button type="button" className="is-guidance" onClick={() => { setExpanded(true); setEditing(item.id); setDraft(item.prompt); }} aria-label="Orientar pedido">Orientar</button>
