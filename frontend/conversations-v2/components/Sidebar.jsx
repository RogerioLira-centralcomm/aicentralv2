  ...
  return <>
    ...
    <aside id="cv-recent-sidebar" className={`cv-recent-sidebar ${open ? 'is-open' : 'is-closing'}`} aria-hidden={!open} inert={!open ? true : undefined} aria-label="Chats recentes">
      ...
      <div className="cv-recent-list">
        {activeProject && <section className="cv-recent-project-section"><header><div><span>Conversas do projeto</span><a href={entityDetailLink(activeProject)}>{activeProject.name || activeProject.title}</a>{activeBrand?.href && <a href={activeBrand.href} className="cv-sidebar-context-link">Marca: {activeBrand.name || activeBrand.title}</a>}</div><b>Últimas 4</b></header>{conversationList(projectConversations, true)}{!loading && !projectConversations.length && <p>{query.trim() ? 'Nenhuma conversa deste projeto corresponde à busca.' : 'Ainda não há conversas deste projeto.'}</p>}</section>}
        ...
        {!activeProject && <section className="cv-recent-project-section"><header><span>Conversas recentes</span><b>Últimas 4</b></header>{conversationList(filtered.slice(0, 4))}{loading && !conversations.length && <div className="cv-recent-loading" aria-label="Carregando conversas"><i/><i/><i/></div>}{!loading && !filtered.length && <p>{query.trim() ? 'Nenhuma conversa corresponde à busca.' : 'Suas conversas aparecerão aqui.'}</p>}</section>}
