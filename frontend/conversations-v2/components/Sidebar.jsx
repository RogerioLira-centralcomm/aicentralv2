...
export function Sidebar({conversations, projects = [], brands = [], activeProjectRef = '', projectResourcesEndpoint = '/workspace/api/v2/projects', activeId, onOpen, open, onClose, loading, openingId}) {
  ...
  const conversationList = items => items.map(item => <button className="cv-conversation-card" key={item.id} type="button" disabled={Boolean(openingId)} onClick={() => onOpen(String(item.id), item.title)} aria-current={String(item.id) === String(activeId) ? 'page' : undefined} title={item.title || 'Conversa sem título'}><span className="cv-conversation-card__copy"><b>{item.title || 'Conversa sem título'}</b>{contextLabel(item, projects, brands) && <small>{contextLabel(item, projects, brands)}</small>}</span>{String(item.id) === String(openingId) && <i/>}</button>);
