  ...
  return <>
    ...
    <aside ref={sidebarRef} id="cv-recent-sidebar" role={window.matchMedia('(max-width: 767px)').matches ? 'dialog' : undefined} aria-modal={window.matchMedia('(max-width: 767px)').matches ? 'true' : undefined} className={`cv-recent-sidebar ${open ? 'is-open' : 'is-closing'}`} aria-hidden={!open} inert={!open ? '' : undefined} aria-label="Chats recentes">
      <div className="cv-mobile-navigation">
        <header><div><strong>Conversas recentes</strong>{activeProject && <small>{activeProject.name || activeProject.title}</small>}</div><button type="button" onClick={onClose} aria-label="Fechar navegação"><Icon name="close" size={18}/></button></header>
        <div className="cv-mobile-navigation__scroll">
          ...
          {(navUrls.docs || navUrls.profile) && <nav aria-label="Outras áreas">{navUrls.docs && <a href={navUrls.docs}>Arquivos</a>}{navUrls.profile && <a href={navUrls.profile}>Configurações</a>}</nav>}
        </div>
      </div>
