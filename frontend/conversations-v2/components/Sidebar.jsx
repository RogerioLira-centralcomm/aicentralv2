  ...
  return <>
    ...
    <aside id="cv-recent-sidebar" className={`cv-recent-sidebar ${open ? 'is-open' : 'is-closing'}`} aria-hidden={!open} inert={!open ? true : undefined} aria-label="Chats recentes">
      ...
      <div className="cv-recent-list">
        {activeProject && <section className="cv-recent-project-section"><header><span>Conversas</span><b>Últimas 10</b></header>{conversationList(projectConversations, true)}{!loading && !projectConversations.length && <p>{query.trim() ? 'Nenhuma conversa deste projeto corresponde à busca.' : 'Ainda não há conversas deste projeto.'}</p>}</section>}
