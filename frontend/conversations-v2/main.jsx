...
import '../cadu-design-system/components/WorkspaceHome.css';
import '../cadu-design-system/components/CaduDock.css';
import '../cadu-design-system/components/WorkspaceEntityPortal.css';
import '../cadu-design-system/components/WorkspaceBrand.css';
...
if (root && bootstrapNode) {
  try {
    ...
    createRoot(root).render(<ThemeProvider skin={workspaceMode ? 'workspace' : 'conversations'} theme={workspaceMode ? 'light' : 'dark'} persistKey={workspaceMode ? 'cadu-workspace-theme' : 'cadu-conversations-theme'} locked={!workspaceMode}>{workspaceMode ? <WorkspaceNotificationsProvider bootstrap={bootstrap}>{surface}</WorkspaceNotificationsProvider> : surface}</ThemeProvider>);
  } catch (error) {
