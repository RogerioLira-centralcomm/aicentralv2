...
function VisualConfirm({detail, data, busy, setBusy, setError, onRefresh}) {
  ...
  const begin = (scope, index) => {
    ...
    for (const metric of scope.metrics || []) {
      ...
      const key = aliases[label]; if (key) metricValues[key] = metric.raw_value;
    }
  ...
  const confirm = async event => {
    ...
    try {
      await json(`/connect/api/v1/reports/imports/${detail.import_file.id}/visual/${active}/confirm?client_id=${data.client.client_id}`, {method:'POST', headers:{'Content-Type':'application/json','X-CSRF-Token':data.csrf}, body:JSON.stringify(draft)});
      setActive(null); await onRefresh();
    } catch (failure) {setError(failure.message);} finally {setBusy(false);}
  };
  return <article className="reports-panel reports-span-three"><div className="reports-panel-head"><h2>Conferir print</h2><span>Imagem original normalizada</span></div><img className="reports-import-image" src={`/connect/api/v1/reports/imports/${detail.import_file.id}/image?client_id=${data.client.client_id}`} alt={`Print enviado: ${detail.import_file.original_name}`} />{scopes.map((scope,index) => {const confirmed = detail.rows.some(row => row.sheet_name === 'Print' && row.source_row === index + 1); const daily = scope.granularity === 'day' && scope.period_start && scope.period_start === scope.period_end; return <div key={index} className="reports-suggestion"><strong>Bloco {index + 1} · {scope.campaign_name || scope.campaign_id || 'Campanha sem identificação'}</strong>{confirmed ? <p>Confirmado e vinculado às observações diárias.</p> : daily ? data.client.role !== 'viewer' && <button type="button" className="reports-text-button" onClick={() => begin(scope,index)}>Conferir e confirmar dia</button> : <p>Período acumulado ou indefinido: mantenha como evidência até existir revisão de intervalo.</p>}{active === index && <form className="reports-form" onSubmit={confirm}><div className="reports-form-pair">{[['platform','Plataforma'],['external_account_id','ID da conta'],['account_name','Nome da conta'],['external_campaign_id','ID da campanha'],['campaign_name','Nome da campanha'],['metric_date','Data ISO'],['currency','Moeda'],['impressions','Impressões'],['clicks','Cliques'],['cost','Custo'],['conversions','Conversões'],['conversion_value','Valor das conversões']].map(([key,label]) => <label key={key}>{label}<input value={draft[key] || ''} onChange={event => setDraft({...draft,[key]:event.target.value})} /></label>)}</div><label>Justificativa<input required maxLength="1000" value={draft.note || ''} onChange={event => setDraft({...draft,note:event.target.value})} placeholder="Conferi os números, IDs e data no print" /></label><button disabled={busy}>Confirmar dados do dia</button></form>}</div>;})}</article>;
}
...
function Imports({data, reloadBootstrap}) {
  ...
  const refresh = async () => {const [body, pending] = await Promise.all([json(base), json(`/connect/api/v1/reports/import-conflicts?client_id=${data.client.client_id}`)]); setReady(body.ready); setItems(body.imports || []); setConflicts(pending.conflicts || []);};
...
function App() {
  ...
  }, [data?.client?.client_id, data?.ready, filters.platform, filters.account, filters.campaign, filters.period, section]);
