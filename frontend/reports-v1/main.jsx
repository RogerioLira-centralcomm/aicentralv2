...
function VisualConfirm({detail, data, busy, setBusy, setError, onRefresh}) {
  ...
  const confirm = async (event, daily) => {
    ...
    try {
      ...
      const payload = daily ? {...shared, metric_date} : {...shared, period_start, period_end};
    ...
    {scopes.map((scope, index) => {
      ...
      return <div key={index} className="reports-suggestion">
        ...
        {active === index && <form className="reports-form" onSubmit={event => confirm(event, daily)}>
          ...
          <button disabled={busy}>Confirmar {daily ? 'dados do dia' : 'total do intervalo'}</button>
        </form>}
      </div>;
    })}
  </article>;
...
function Imports({data, reloadBootstrap}) {
  ...
  const refresh = async () => {const [body, pending, ranges] = await Promise.all([json(base), json(`/connect/api/v1/reports/import-conflicts?client_id=${data.client.client_id}`), json(`/connect/api/v1/reports/import-ranges?client_id=${data.client.client_id}`)]); setReady(body.ready); setItems(body.imports || []); setConflicts(pending.conflicts || []); setRangeSnapshots(ranges.snapshots || []);};
