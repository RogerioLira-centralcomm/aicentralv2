...
function Campaigns({data, save, busy}) {
  ...
  useEffect(()=>{const sync=()=>setCampaignId(new URLSearchParams(location.search).get('campaign_id')||new URLSearchParams(location.hash.split('?')[1]||'').get('campaign_id')||'');addEventListener('popstate',sync);return()=>removeEventListener('popstate',sync);},[]);
  ...
  if (campaignId) return <CampaignDetail data={data} detail={campaignDetail} error={detailError} tab={tab} setTab={setTab} close={closeCampaign} />;
...
function CampaignDetail({data, detail, error, tab, setTab, close}) {
  ...
  const valid=detail.metrics.filter(item=>item.value_numeric!=null);
  const latest=totals.reduce((value,item)=>!value||item.latest_date>value?item.latest_date:value,'');
  const metricValue=key=>totals.filter(item=>item.metric_key===key).reduce((sum,item)=>sum+Number(item.total_value||0),0);
  const metricCount=key=>totals.filter(item=>item.metric_key===key).reduce((sum,item)=>sum+Number(item.observation_count||0),0);
  ...
  return <section className="reports-grid reports-grid--four reports-campaign-detail">
    ...
    <nav className="reports-panel reports-span-four reports-campaign-tabs" aria-label="Seções da campanha">{[['overview','Visão geral'],['metrics','Métricas'],['reports','Relatórios'],['imports','Histórico de dados'],['settings','Configurações']].map(([key,label])=><button type="button" className={tab===key?'is-active':''} onClick={()=>setTab(key)} key={key}>{label}</button>)}</nav>
    {tab==='overview'&&<><Kpi label="Impressões" value={integer(metricValue('impressions'))} detail={`${metricCount('impressions')} observações conciliadas`}/><Kpi label="Cliques" value={integer(metricValue('clicks'))} detail={`${metricCount('clicks')} observações conciliadas`}/><Kpi label="Conversões" value={decimal(metricValue('conversions'))} detail="informadas pelos canais"/><Kpi label="Último dado" value={shortDate(latest)} detail="data mais recente entre as métricas importadas"/><article className="reports-panel reports-span-two"><div className="reports-panel-head"><h3>Métricas principais</h3><span>Totais conciliados · moedas separadas</span></div><div className="reports-table-wrap"><table><thead><tr><th>Métrica</th><th>Total importado</th><th>Registros</th><th>Conflitos</th></tr></thead><tbody>{Object.entries(metricLabels).flatMap(([key,label])=>totals.filter(item=>item.metric_key===key).map(item=><tr key={`${key}:${item.currency||'none'}`}><td>{label}</td><td>{key==='cost'||key==='conversion_value'?amount(item.total_value,item.currency):integer(item.total_value)}</td><td>{item.observation_count}</td><td>{item.conflict_count}</td></tr>))}</tbody></table>{!totals.length&&<Empty message="Nenhuma métrica diária importada para esta campanha."/>}</div></article><article className="reports-panel reports-span-two"><div className="reports-panel-head"><h3>Dados personalizados</h3><span>{detail.custom_values.length} pares chave/valor</span></div>{detail.custom_values.length?detail.custom_values.slice(0,8).map((item,index)=><div className="reports-row" key={`${item.metric_key}:${item.metric_date}:${index}`}><span>{item.metric_label} · {item.channel}<small>{item.metric_key} · {shortDate(item.metric_date)}</small></span><b>{item.value_numeric} {item.currency||item.unit}</b></div>):<Empty message="Dados extras importados aparecerão associados a esta campanha."/>}</article></>}
    {tab==='metrics'&&<article className="reports-panel reports-span-four"><div className="reports-panel-head"><h3>Métricas por data</h3><span>{detail.metrics.length} observações</span></div>{detail.metrics.length?<div className="reports-table-wrap"><table><thead><tr><th>Data</th><th>Métrica</th><th>Valor</th><th>Moeda</th><th>Estado</th></tr></thead><tbody>{detail.metrics.map((item,index)=><tr key={`${item.metric_date}:${item.metric_key}:${index}`}><td>{shortDate(item.metric_date)}</td><td>{metricLabels[item.metric_key]||item.metric_key}</td><td>{item.value_numeric??'Em conflito'}</td><td>{item.currency||'—'}</td><td>{item.version_count>1?'Revisar divergência':'Conciliado'}</td></tr>)}</tbody></table></div>:<Empty message="Ainda não há métricas diárias importadas para esta campanha."/>}</article>}
