...
function Reports({data, save, busy}) {
  ...
  return <><section className="reports-grid reports-grid--four"><article className="reports-panel reports-span-three"><div className="reports-panel-head"><h2>Biblioteca</h2><span>{data.reports.length} relatórios</span></div><div className="reports-grid reports-grid--three">{data.reports.length ? data.reports.map(item => <a className="reports-panel reports-report-card" key={item.id} href={`/connect/relatorios?report_id=${item.id}`}><span>Relatório · v{item.revision}</span><h2>{item.campaign_name}</h2><p>{item.project_ref || 'Espaço independente'}</p><small>Atualizado em {shortDate(item.updated_at)}</small></a>) : <Empty message="Crie um relatório independente ou associado a uma campanha." />}</div></article><article className="reports-panel"><div className="reports-panel-head"><h2>Novo relatório</h2><span>Reports</span></div><form className="reports-form" onSubmit={submit}><label>Nome<input required maxLength="200" value={form.campaign_name} onChange={event => setForm({...form, campaign_name: event.target.value})} placeholder="Ex.: Resultado de setembro" /></label><label>Campanha (opcional)<select value={form.media_campaign_id} onChange={event => setForm({...form, media_campaign_id: event.target.value})}><option value="">Sem campanha vinculada</option>{data.campaigns.map(item => <option key={item.id} value={item.id}>{item.account_name} · {item.name}</option>)}</select></label><button disabled={busy} type="submit">Criar relatório</button></form></article></section></>;
}
...
function Links({data, save, busy}) {
  ...
  const suggest = async runId => {try {const body = await save(`/link-tests/${runId}/suggest-campaign`, {}, false); setSuggestion(body);} catch (_) { /* Global error banner shows the failure. */ }};
  return <section className="reports-grid reports-grid--three"><article className="reports-panel"><div className="reports-panel-head"><h2>Testar destino</h2><span>Link Tester</span></div><form className="reports-form" onSubmit={submit}>
    ...
    <article className="reports-panel reports-span-three"><div className="reports-panel-head"><h2>Histórico</h2><span>{data.link_tests.length} recentes</span></div>{aiStatus && !aiStatus.configured && <p className="reports-suggestion">TypeSafe ainda não está configurada nas Integrações do Cadu. IDs exatos de campanha continuam reconhecidos por regra; a revisão semântica fica disponível após configurar a chave.</p>}{suggestion && <div className="reports-suggestion">{suggestion.suggestion ? <>Campanha sugerida: <strong>{suggestion.suggestion.name}</strong> · {suggestion.model === 'exact_id' ? 'ID externo exato' : `confiança ${Math.round((suggestion.confidence || 0) * 100)}%`}. Revise antes de associar.</> : (suggestion.reason || 'Nenhuma campanha sugerida.')}{suggestion.page_role && <span className="reports-suggestion-role">Tipo provável de página: {({landing: 'entrada', form: 'formulário', thank_you: 'obrigado', content: 'conteúdo', unknown: 'indefinido'})[suggestion.page_role] || suggestion.page_role}. Essa classificação também precisa de revisão.</span>}</div>}{data.link_tests.length ? <div className="reports-table-wrap"><table><thead><tr><th>Destino</th><th>Tipo</th><th>Resultado</th><th>Data</th><th>Campanha</th></tr></thead><tbody>{data.link_tests.map(item => <tr key={item.id}><td>{item.final_url}</td><td>{item.mode}</td><td>{item.score}/100 · {item.status_label}</td><td>{shortDate(item.created_at)}</td><td><button className="reports-text-button" disabled={busy} onClick={() => suggest(item.id)}>Sugerir</button></td></tr>)}</tbody></table></div> : <Empty message="Os testes realizados neste cliente aparecerão aqui." />}</article>
  </section>;
}
...
function Monitor({data, save, busy}) {
  ...
  const create = async event => {
    ...
    try {
      ...
      const created = await save('/ingest-keys', {label, source_kind: sourceKind}, false);
      if (sourceKind === 'google_ads_script') {
        setScript(template.replace('__CADU_INGEST_URL__', `${location.origin}/connect/api/v1/reports/ingest/google-ads`).replace('__CADU_API_KEY__', created.token));
      } else {
...
function App() {
  ...
  return <div className="reports-shell">
    ...
    <main className="reports-main"><header className="reports-header"><div><p>REPORTS / {TITLES[section] || 'Visão geral'}</p><h1>{TITLES[section] || 'Visão geral'}</h1></div><div className="reports-header-actions"><select className="reports-client-pill" aria-label="Cliente" value={data?.client?.client_id || ''} onChange={event => {location.href = `/connect/app?client_id=${encodeURIComponent(event.target.value)}#${section}`;}}>{(data?.clients || []).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select><a href="/connect/relatorios">Biblioteca atual ↗</a></div></header>
      ...
      <div className="reports-content">{error && <div className="reports-error" role="alert">{error}</div>}{!data ? <Empty message="Carregando Reports…" /> : !data.ready ? <Empty message="A base de Reports V1 ainda precisa da migração de dados." /> : section === 'accounts' ? <Accounts data={selected} save={save} busy={busy} /> : section === 'campaigns' ? <Campaigns data={selected} save={save} busy={busy} /> : section === 'reports' ? <Reports data={selected} save={save} busy={busy} /> : section === 'links' ? <Links data={selected} save={save} busy={busy} /> : section === 'monitor' ? <Monitor data={data} save={save} busy={busy} /> : section === 'flow' ? <Flow data={data} save={save} busy={busy} filters={filters} /> : <Overview data={selected} metrics={metrics} />}</div>
    </main>
  </div>;
}
