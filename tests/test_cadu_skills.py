...
class CaduSkillsTest(TestCase):
    ...
    def test_public_marketplace_and_detail_do_not_require_login(self, _event, _managed):
        ...
        self.assertIn("O que você pode fazer com esta skill.", detail.get_data(as_text=True))
    ...
    def test_public_learning_and_content_pages_explain_skills(self):
        ...
        self.assertIn('href="/skills/metodos"', learn.get_data(as_text=True))
        ...
        self.assertIn("Quando conversar com o agente", content.get_data(as_text=True))
        article = client.get("/skills/aprender/contexto-que-melhora-resposta")
        self.assertEqual(article.status_code, 200)
        self.assertIn("O contexto que faz", article.get_data(as_text=True))
        filtered = client.get("/skills/metodos?q=audiencia")
        self.assertEqual(filtered.status_code, 200)
        self.assertIn("Inteligência de audiência", filtered.get_data(as_text=True))
        empty = client.get("/skills/metodos?q=naoexiste")
        self.assertIn("Nenhum método corresponde", empty.get_data(as_text=True))
    ...
    def test_logged_user_enters_the_shared_skills_catalog(self, _skills, _customizations, _credit):
        ...
        self.assertEqual(response.status_code, 200)
        html = response.get_data(as_text=True)
        self.assertIn("O que precisa ganhar clareza agora?", html)
        self.assertIn("Métodos do seu trabalho", html)
        self.assertIn('class="skills-topbar"', html)
        self.assertIn('class="skills-topbar__account"', html)
        self.assertNotIn('cadu-app-sidebar cadu-skills-sidebar', html)
        self.assertNotIn('data-cadu-sidebar-mobile-toggle', html)
    ...
    def test_public_skill_is_an_editorial_installation_page(self, _event):
        ...
        html = response.get_data(as_text=True)
        self.assertIn("O que este método ajuda a decidir", html)
        self.assertIn("Leve o método para um projeto.", html)
        self.assertNotIn("Agente de teste", html)
        personal = client.get("/skills/personalizar")
        self.assertEqual(personal.status_code, 200)
        self.assertIn("O método continua. O contexto passa a acompanhar.", personal.get_data(as_text=True))
