...
class CrmV3ActivityUiContractTest(unittest.TestCase):
    ...
    def test_ai_is_progressive_and_keeps_configuration_collapsed(self):
        ...
        self.assertIn('data-chip-group="canal_produto" data-chip-optional', self.template)
        self.assertIn("Kit do canal", self.template)
        ...
        self.assertIn('class="cx-atividade-ia-products"', self.template)
        self.assertIn("O kit entra com 1 a 3 números", self.template)
        ...
        self.assertIn("data-canal-filter", self.template)
