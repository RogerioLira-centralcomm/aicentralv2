...
class CrmV3ActivityUiContractTest(unittest.TestCase):
    ...
    def test_ai_is_progressive_and_keeps_configuration_collapsed(self):
        ...
        self.assertIn("if (raw.canal_produto) payload.canal_produto = raw.canal_produto", self.js)
        self.assertIn("canal_produto: 1", self.js)
        self.assertIn("payload.tipo === 'reuniao' && meetingEditor && payload.hora", self.js)
        self.assertNotIn("if (payload.tipo === 'reuniao' && !payload.hora) return Promise.resolve();", self.js)
        self.assertIn("if (!atividadeRef.id && form._flushAtividadeSave)", self.js)
