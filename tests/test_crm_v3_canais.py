...
class CrmV3CanaisCatalogTest(unittest.TestCase):
    ...
    def test_sidebar_ordena_canais_alfabeticamente(self):
        ...
        self.assertIn("cx-canais-group", js)
        self.assertIn("cx-canais-bloco", js)
        self.assertIn("cx-canais-open", js)
        self.assertNotIn("<small>' +", js.split("function paint(canais, ativo)")[1].split("function persistCanaisGrupos")[0])
    ...
    def test_catalogo_traz_base_e_logo_local(self):
        ...
        self.assertNotIn("google-dv360", slugs)
        self.assertNotIn("waze", slugs)
        ...
        self.assertEqual(resolver_canal("SBT")["logo"], "/static/images/canais/sbt.png")
    ...
    def test_grupos_sao_os_oito_na_ordem(self):
        ...
        presentes = {item["categoria"] for item in listar_canais() if item.get("categoria")}
        self.assertEqual(grupos, [nome for nome in GRUPOS if nome in presentes])
        self.assertIn("Portais", grupos)
        self.assertIn("Streaming", grupos)
        self.assertIn("Mobilidade", grupos)
        self.assertNotIn("google-dv360", {item["slug"] for item in listar_canais()})
    ...
    def test_sidebar_tem_sessao_playbook_e_thumbs(self):
        ...
        self.assertNotIn("data-canais-busca", drawer)
        self.assertNotIn("data-canais-cats", drawer)
        self.assertIn("data-canais-grupo-toggle", js)
        self.assertIn("data-canais-playbook", js)
        self.assertIn("data-canais-gerar", js)
        self.assertIn("contextoCanalAtividade", js)
        self.assertIn("rascunhoAtividadeCanal", js)
        self.assertIn("data_prazo", js)
        self.assertIn("fromCanais: true", js)
        self.assertIn("Criar atividade", js)
        self.assertIn("Abra um cliente ou agência para gravar a atividade", js)
        self.assertIn("gerarRoteiro: true", js)
        self.assertIn("forceNew: true", js)
        self.assertIn("/atividades", js)
        self.assertIn("refreshAtividadeLists(aberto.id)", js)
        self.assertNotIn("data-canais-abrir-atividade", js)
        ...
        self.assertIn(".cx-canais-item[hidden]", css)
        self.assertIn("cx-canais-group", css)
        self.assertIn("cx-canais-commit", css)
    ...
    def test_api_canais_devolve_contrato_enriquecido(self):
        ...
        self.assertNotIn("google-dv360", slugs)
        self.assertNotIn("waze", slugs)
        self.assertEqual(payload.get("grupos"), grupos_canais())
