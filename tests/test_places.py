...
class PlacesCatalogTest(unittest.TestCase):
    ...
    def test_every_public_place_has_audience_plan_demography_and_top_15(self):
        for index, item in enumerate(SEED_PLACES, start=1):
            view = public_view(dict(item, id=index))
            self.assertEqual(len(view["channel_ranking"]), 15, item["slug"])
            self.assertTrue(all(row["short"] for row in view["channel_ranking"]), item["slug"])
            self.assertTrue(all(row["color"].startswith("#") for row in view["channel_ranking"]), item["slug"])
            self.assertEqual(len(view["audience_plan"]["steps"]), 4, item["slug"])
            self.assertTrue(view["audience_plan"]["reuse_note"], item["slug"])
            self.assertTrue(view["demographics"]["age"], item["slug"])
            self.assertTrue(view["demographics"]["income"], item["slug"])
            self.assertGreaterEqual(len(view["target_audience"]), 4, item["slug"])
...
class PlacesPublicRoutesTest(unittest.TestCase):
    ...
    def test_public_place_renders_catchment(self):
        ...
        self.assertIn("Ir para os recortes", html)
        self.assertIn("O SDU liga hotel", html)
        self.assertIn("cc-public-hero", html)
        self.assertIn("cc-recort-card", html)
        self.assertIn("cc-audience-cycle", html)
        self.assertIn("cc-channel-ranking", html)
        self.assertIn("cc-channel-mark", html)
        self.assertIn("Abrir site oficial de", html)
        self.assertNotIn("cc-brief", html)
        self.assertIn("cc-public-close", html)
        self.assertIn("Valide o plano com quem enviou este material", html)
        self.assertNotIn("data-ask-form", html)
        self.assertNotIn("Pedir o recorte", html)
        self.assertIn("santos-dumont-hero", html)
        self.assertIn("não é presença no lugar", html)
    ...
    def test_public_place_shows_point_photo_in_the_sheet(self):
        ...
        html = response.get_data(as_text=True)
        self.assertIn("cc-recort-card", html)
        self.assertIn("cc-public-photos", html)
        self.assertNotIn("cc-point-table-photo", html)
    ...
    def test_confins_public_shows_points_and_offer(self):
        ...
        self.assertNotIn("Índices disponíveis sob consulta", html)
        self.assertIn("MG-010", html)
        self.assertIn("Escolha o recorte que faz sentido", html)
        self.assertIn("No sítio", html)
        self.assertIn("-19.630503", html)
        self.assertIn("Top 15 apps e sites", html)
        self.assertIn("A campanha cria uma audiência que continua útil", html)
        self.assertIn("Investimento estimado · 4 semanas", html)
