...
class DeliveryTest(unittest.TestCase):
    ...
    def test_formats_download_and_anonymous_html(self):
        ...
        with patch('aicentralv2.creative_format_lab.swap_routes._http',return_value=http),patch('aicentralv2.creative_media.studio_media.resolve_clip',return_value=self.voiced),patch.object(studio._POOL,'submit',side_effect=lambda fn,*args:fn(*args)):
            for index,kind in enumerate(['mp4','gif','html'],1):
                ...
                with client.get(status['download_url']) as result:
                ...
                if kind=='html':
                    ...
                    self.assertNotEqual(anonymous.get(status['download_url']).status_code,200)
